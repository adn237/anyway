...
this.visible_=false;};ClusterIcon.prototype.show=function(){if(this.div_){var pos=this.getPosFromLatLng_(this.center_);this.div_.style.cssText=this.createCss(pos);this.div_.innerHTML="<div style='"+"position: absolute;"+"top: "+this.anchorText_[0]+"px;"+"left: "+this.anchorText_[1]+"px;"+"color: black;"+"font-weight: bold;"+"font-size: 15px;"+"font-family: "+"Alef, sans-serif;"+"border: "+this.width_+"px solid #D63230;"+"border-radius: 50%;"+"text-align: center;"+"width: 32px;"+"line-height: 32px;"+"vertical-align: middle;"+"background-color:"+"white;"+"opacity:"+"0.7;"+"'>"+this.sums_.text+"</div>";if(typeof this.sums_.title==="undefined"||this.sums_.title===""){this.div_.title=this.cluster_.getMarkerClusterer().getTitle();}else{this.div_.title=this.sums_.title;}
this.div_.style.display="";}
this.visible_=true;};ClusterIcon.prototype.useStyle=function(sums){this.sums_=sums;var index=Math.max(0,sums.index-1);index=Math.min(this.styles_.length-1,index);var style=this.styles_[index];this.height_=style.height;this.width_=style.width;this.anchorText_=style.anchorText||[0,0];this.anchorIcon_=style.anchorIcon||[parseInt(this.height_/2,10),parseInt(this.width_/2,10)];this.textColor_=style.textColor||"black";this.textSize_=style.textSize||11;this.textDecoration_=style.textDecoration||"none";this.fontWeight_=style.fontWeight||"bold";this.fontStyle_=style.fontStyle||"normal";this.fontFamily_=style.fontFamily||"Arial,sans-serif";this.backgroundPosition_=style.backgroundPosition||"0 0";};ClusterIcon.prototype.setCenter=function(center){this.center_=center;};ClusterIcon.prototype.createCss=function(pos){var style=[];style.push("cursor: pointer;");style.push("position: absolute; top: "+pos.y+"px; left: "+pos.x+"px;");style.push("width: "+this.width_+"px; height: "+this.height_+"px;");return style.join("");};ClusterIcon.prototype.getPosFromLatLng_=function(latlng){var pos=this.getProjection().fromLatLngToDivPixel(latlng);pos.x-=this.anchorIcon_[1];pos.y-=this.anchorIcon_[0];pos.x=parseInt(pos.x,10);pos.y=parseInt(pos.y,10);return pos;};function Cluster(mc){this.markerClusterer_=mc;this.map_=mc.getMap();this.gridSize_=mc.getGridSize();this.minClusterSize_=mc.getMinimumClusterSize();this.averageCenter_=mc.getAverageCenter();this.markers_=[];this.center_=null;this.bounds_=null;this.clusterIcon_=new ClusterIcon(this,mc.getStyles());}
...
index=Math.min(index,numStyles);return{text:count,index:index,title:title};};MarkerClusterer.BATCH_SIZE=2000;MarkerClusterer.BATCH_SIZE_IE=500;MarkerClusterer.IMAGE_PATH="http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclustererplus/images/m";MarkerClusterer.IMAGE_EXTENSION="png";MarkerClusterer.IMAGE_SIZES=[53,56,66,78,90];MarkerClusterer.prototype.addCluster=function(clat,clng,csize){this.setZoomOnClick(true);if(typeof this.aAddClusterIcons=="undefined"){this.aAddClusterIcons=[];}
this.activeMap_=this.getMap();var clusterlocation=new google.maps.LatLng(clat,clng);var CI=new ClusterIcon(new Cluster(this));var index=0;var dv=csize;while(dv!==0){dv=parseInt(dv/10,10);index++;}
var style=this.styles_[index-1];$.extend(CI,{sums_:{text:csize,index:index},url_:style['url'],width_:style['width'],size_:style['size'],anchorIcon_:[clat,clng],anchorText_:[0,0],backgroundPosition_:'0 0'});CI.setCenter(clusterlocation);CI.cluster_.center_=clusterlocation;CI.cluster_.calculateBounds_();CI.setMap(this.activeMap_);CI.show();this.aAddClusterIcons.push(CI);};MarkerClusterer.prototype.removeClusters=function(clat,clng,csize){if(typeof this.aAddClusterIcons=="undefined"){this.aAddClusterIcons=[];}
...
return((r[1].length===0)?r[0]:null);};};$D.parseExact=function(s,fx){return $D.getParseFunction(fx)(s);};}());!function($){var DateRangePicker=function(element,options,cb){var hasOptions=typeof options=='object'
var localeObject;this.startDate=Date.today();this.endDate=Date.today();this.minDate=false;this.maxDate=false;this.changed=false;this.cleared=false;this.ranges={};this.opens='right';this.cb=function(){};this.format='MM/dd/yyyy';this.separator=' - ';this.showWeekNumbers=false;this.buttonClasses=['btn-success'];this.locale={applyLabel:'החל',clearLabel:"נקה",fromLabel:'מ',toLabel:'עד',weekLabel:'W',customRangeLabel:'בחר תאריך',daysOfWeek:Date.CultureInfo.shortestDayNames,monthNames:Date.CultureInfo.monthNames,firstDay:0};localeObject=this.locale;this.leftCalendar={month:Date.today().set({day:1,month:this.startDate.getMonth(),year:this.startDate.getFullYear()}),calendar:Array()};this.rightCalendar={month:Date.today().set({day:1,month:this.endDate.getMonth(),year:this.endDate.getFullYear()}),calendar:Array()};this.element=$(element);if(this.element.hasClass('pull-right'))
...
var DRPTemplate='<div class="daterangepicker dropdown-menu">'+'<div class="calendar left"></div>'+'<div class="calendar right"></div>'+'<div class="ranges">'+'<div class="range_inputs">'+'<div><div class="daterangepicker_start_input" style="float: right; clear: both">'+'<label for="daterangepicker_start">'+this.locale.fromLabel+'</label>'+'<input class="input-mini" type="text" name="daterangepicker_start" value="" disabled="disabled" />'+'</div>'+'<div class="daterangepicker_end_input" style="float: right; padding-right: 11px">'+'<label for="daterangepicker_end">'+this.locale.toLabel+'</label>'+'<input class="input-mini" type="text" name="daterangepicker_end" value="" disabled="disabled" />'+'</div></div>'+'<div><button class="btn btn-small btn-success applyBtn" disabled="disabled">'+this.locale.applyLabel+'</button>&nbsp;'+'<button class="btn btn-small clearBtn">'+this.locale.clearLabel+'</button></div>'+'</div>'+'</div>'+'</div>';this.container=$(DRPTemplate).appendTo('body');if(hasOptions){if(typeof options.format=='string')
this.format=options.format;if(typeof options.separator=='string')
this.separator=options.separator;if(typeof options.startDate=='string')
this.startDate=Date.parse(options.startDate,this.format);if(typeof options.endDate=='string')
this.endDate=Date.parse(options.endDate,this.format);if(typeof options.minDate=='string')
this.minDate=Date.parse(options.minDate,this.format);if(typeof options.maxDate=='string')
this.maxDate=Date.parse(options.maxDate,this.format);if(typeof options.startDate=='object')
...
if(typeof options.locale=='object'){if(typeof options.locale.firstDay=='number'){this.locale.firstDay=options.locale.firstDay;var iterator=options.locale.firstDay;while(iterator>0){this.locale.daysOfWeek.push(this.locale.daysOfWeek.shift());iterator--;}}}
...
this.container.find('.calendar').show();if(typeof cb=='function')
...
return calendar;},renderCalendar:function(calendar,selected,minDate,maxDate){var html='<table class="table-condensed">';html+='<thead>';html+='<tr>';if(this.showWeekNumbers)
html+='<th></th>';if(!minDate||minDate<calendar[1][1])
{html+='<th class="prev available"><i class="icon-arrow-left"></i></th>';}
else
{html+='<th></th>';}
html+='<th colspan="5" style="width: auto">'+this.locale.monthNames[calendar[1][1].getMonth()]+calendar[1][1].toString(" yyyy")+'</th>';if(!maxDate||maxDate>calendar[1][1])
{html+='<th class="next available"><i class="icon-arrow-right"></i></th>';}
else
{html+='<th></th>';}
html+='</tr>';html+='<tr>';if(this.showWeekNumbers)
html+='<th class="week">'+this.locale.weekLabel+'</th>';$.each(this.locale.daysOfWeek,function(index,dayOfWeek){html+='<th>'+dayOfWeek+'</th>';});html+='</tr>';html+='</thead>';html+='<tbody>';for(var row=0;row<6;row++){html+='<tr>';if(this.showWeekNumbers)
html+='<td class="week">'+calendar[row][0].getWeek()+'</td>';for(var col=0;col<7;col++){var cname='available ';cname+=(calendar[row][col].getMonth()==calendar[1][1].getMonth())?'':'off';selected.setHours(0,0,0,0);if((minDate&&calendar[row][col]<minDate)||(maxDate&&calendar[row][col]>maxDate))
{cname=' off disabled ';}
else if(calendar[row][col].equals(selected))
{cname+=' active ';if(calendar[row][col].equals(this.startDate)){cname+=' start-date ';}
if(calendar[row][col].equals(this.endDate)){cname+=' end-date ';}}
else if(calendar[row][col]>=this.startDate&&calendar[row][col]<=this.endDate)
{cname+=' in-range ';if(calendar[row][col].equals(this.startDate)){cname+=' start-date ';}
if(calendar[row][col].equals(this.endDate)){cname+=' end-date ';}}
var title='r'+row+'c'+col;html+='<td class="'+cname.replace(/\s+/g,' ').replace(/^\s?(.*?)\s?$/,'$1')+'" title="'+title+'">'+calendar[row][col].getDate()+'</td>';}
html+='</tr>';}
...
el.data('daterangepicker',new DateRangePicker(el,options,cb));});return this;};}(window.jQuery);JSITM={};JSITM.LatLng=function(lat,lng,alt,precision){this.lat=lat;this.lng=lng;this.alt=alt||0;this.precision=precision||5;}
JSITM.LatLng.prototype.toString=function(){function round(n,precision){var m=Math.pow(10,precision||0);return Math.round(n*m)/m;}
return round(this.lat,this.precision)+" "+round(this.lng,this.precision);}
JSITM.LatLng.prototype.convertGrid=function(from,to,translation){var point=from.latLngToPoint(this,0);var translated=translation.translate(point);return to.pointToLatLng(translated);}
JSITM.latlngFromString=function(s){var pattern=new RegExp("^(-?\\d+(?:\\.\\d*)?)(?:(?:\\s*[:,]?\\s*)|\\s+)(-?\\d+(?:\\.\\d*)?)$","i")
var latlng=s.match(pattern);if(latlng){var lat=parseFloat(latlng[1],10);var lng=parseFloat(latlng[2],10);return new JSITM.LatLng(lat,lng);}
throw("could not parse latlng")}
JSITM.Point=function(x,y,z){this.y=y;this.x=x;this.z=z||0;}
JSITM.Point.prototype.toString=function(){return Math.round(this.x)+" "+Math.round(this.y);}
JSITM.Translation=function(dx,dy,dz){this.dx=dx;this.dy=dy;this.dz=dz;}
JSITM.Translation.prototype.translate=function(point){return new JSITM.Point(point.x+this.dx,point.y+this.dy,point.z+this.dz);}
JSITM.Translation.prototype.inverse=function(){return new JSITM.Translation(-this.dx,-this.dy,-this.dz);}
JSITM.Ellipsoid=function(a,b){this.a=a;this.b=b;this.e2=(Math.pow(a,2)-Math.pow(b,2))/Math.pow(a,2);}
JSITM.Ellipsoid.prototype.pointToLatLng=function(point){var RootXYSqr=Math.sqrt(Math.pow(point.x,2)+Math.pow(point.y,2));var radlat1=Math.atan2(point.z,(RootXYSqr*(1-this.e2)));do{var V=this.a/(Math.sqrt(1-(this.e2*Math.pow(Math.sin(radlat1),2))));var radlat2=Math.atan2((point.z+(this.e2*V*(Math.sin(radlat1)))),RootXYSqr);if(Math.abs(radlat1-radlat2)>0.000000001){radlat1=radlat2;}
else{break;}}
while(true);var lat=radlat2*(180/Math.PI);var lng=Math.atan2(point.y,point.x)*(180/Math.PI);return new JSITM.LatLng(lat,lng);}
JSITM.Ellipsoid.prototype.latLngToPoint=function(latlng){var radlat=latlng.lat*(Math.PI/180);var radlng=latlng.lng*(Math.PI/180);var V=this.a/(Math.sqrt(1-(this.e2*(Math.pow(Math.sin(radlat),2)))));var x=(V+latlng.alt)*(Math.cos(radlat))*(Math.cos(radlng));var y=(V+latlng.alt)*(Math.cos(radlat))*(Math.sin(radlng));var z=((V*(1-this.e2))+latlng.alt)*(Math.sin(radlat));return new JSITM.Point(x,y,z);}
JSITM.TM=function(ellipsoid,e0,n0,f0,lat0,lng0){this.ellipsoid=ellipsoid;this.e0=e0;this.n0=n0;this.f0=f0;this.lat0=lat0;this.lng0=lng0;this.radlat0=lat0*(Math.PI/180);this.radlng0=lng0*(Math.PI/180);this.af0=ellipsoid.a*f0;this.bf0=ellipsoid.b*f0;this.e2=(Math.pow(this.af0,2)-Math.pow(this.bf0,2))/Math.pow(this.af0,2);this.n=(this.af0-this.bf0)/(this.af0+this.bf0);this.n2=this.n*this.n;this.n3=this.n2*this.n;}
JSITM.TM.prototype.Marc=function(radlat){return(this.bf0*(((1+this.n+((5/4)*this.n2)+((5/4)*this.n3))*(radlat-this.radlat0))-
...
JSITM.TM.prototype.InitialLat=function(y){var radlat1=((y-this.n0)/this.af0)+this.radlat0;var M=this.Marc(radlat1);var radlat2=((y-this.n0-M)/this.af0)+radlat1;while(Math.abs(y-this.n0-M)>0.00001){radlat2=((y-this.n0-M)/this.af0)+radlat1;M=this.Marc(radlat2);radlat1=radlat2;}
return radlat2;}
JSITM.TM.prototype.unproject=function(point){var Et=point.x-this.e0;var PHId=this.InitialLat(point.y);var nu=this.af0/(Math.sqrt(1-(this.e2*(Math.pow(Math.sin(PHId),2)))));var rho=(nu*(1-this.e2))/(1-(this.e2*Math.pow(Math.sin(PHId),2)));var eta2=(nu/rho)-1;var VII=(Math.tan(PHId))/(2*rho*nu);var VIII=((Math.tan(PHId))/(24*rho*Math.pow(nu,3)))*(5+(3*Math.pow(Math.tan(PHId),2))+eta2-(9*eta2*(Math.pow(Math.tan(PHId),2))));var IX=(Math.tan(PHId)/(720*rho*Math.pow(nu,5)))*(61+(90*Math.pow(Math.tan(PHId),2))+(45*(Math.pow(Math.tan(PHId),4))));var lat=(180/Math.PI)*(PHId-(Math.pow(Et,2)*VII)+(Math.pow(Et,4)*VIII)-(Math.pow(Et,6)*IX));var X=(Math.pow(Math.cos(PHId),-1))/nu;var XI=((Math.pow(Math.cos(PHId),-1))/(6*Math.pow(nu,3)))*((nu/rho)+(2*(Math.pow(Math.tan(PHId),2))));var XII=((Math.pow(Math.cos(PHId),-1))/(120*Math.pow(nu,5)))*(5+(28*(Math.pow(Math.tan(PHId),2)))+(24*(Math.pow(Math.tan(PHId),4))));var XIIA=((Math.pow(Math.cos(PHId),-1))/(5040*Math.pow(nu,7)))*(61+(662*(Math.pow(Math.tan(PHId),2)))+(1320*(Math.pow(Math.tan(PHId),4)))+(720*(Math.pow(Math.tan(PHId),6))));var lng=(180/Math.PI)*(this.radlng0+(Et*X)-(Math.pow(Et,3)*XI)+(Math.pow(Et,5)*XII)-(Math.pow(Et,7)*XIIA));var latlng=new JSITM.LatLng(lat,lng);return(latlng);}
JSITM.TM.prototype.project=function(latlng){var RadPHI=latlng.lat*(Math.PI/180);var RadLAM=latlng.lng*(Math.PI/180);var nu=this.af0/(Math.sqrt(1-(this.e2*Math.pow(Math.sin(RadPHI),2))));var rho=(nu*(1-this.e2))/(1-(this.e2*Math.pow(Math.sin(RadPHI),2)));var eta2=(nu/rho)-1;var p=RadLAM-this.radlng0;var M=this.Marc(RadPHI);var I=M+this.n0;var II=(nu/2)*(Math.sin(RadPHI))*(Math.cos(RadPHI));var III=((nu/24)*(Math.sin(RadPHI))*(Math.pow(Math.cos(RadPHI),3)))*(5-(Math.pow(Math.tan(RadPHI),2))+(9*eta2));var IIIA=((nu/720)*(Math.sin(RadPHI))*(Math.pow(Math.cos(RadPHI),5)))*(61-(58*(Math.pow(Math.tan(RadPHI),2)))+(Math.pow(Math.tan(RadPHI),4)));var y=I+(Math.pow(p,2)*II)+(Math.pow(p,4)*III)+(Math.pow(p,6)*IIIA);var IV=nu*(Math.cos(RadPHI));var V=(nu/6)*(Math.pow(Math.cos(RadPHI),3))*((nu/rho)-(Math.pow(Math.tan(RadPHI),2)));var VI=(nu/120)*(Math.pow(Math.cos(RadPHI),5))*(5-(18*(Math.pow(Math.tan(RadPHI),2)))+(Math.pow(Math.tan(RadPHI),4))+(14*eta2)-(58*(Math.pow(Math.tan(RadPHI),2))*eta2));var x=this.e0+(p*IV)+(Math.pow(p,3)*V)+(Math.pow(p,5)*VI);return new JSITM.Point(x,y,0)}
JSITM.GRS80=new JSITM.Ellipsoid(6378137,6356752.31414);JSITM.WGS84=new JSITM.Ellipsoid(6378137,6356752.314245);JSITM.GRS80toWGS84=new JSITM.Translation(-48,55,52);JSITM.WGS84toGRS80=JSITM.GRS80toWGS84.inverse();JSITM.ITM=new JSITM.TM(JSITM.GRS80,219529.58400,626907.38999,1.000006700000000,31.734394,35.204517);JSITM.point2ItmRef=function(point,precision){function zeropad(num,len){var str=new String(num);while(str.length<len){str='0'+str;}
return str;}
var p=precision||6;if(p<3)
p=3;if(p>6)
p=6;var div=Math.pow(10,(6-p));var east=Math.round(point.x/div);var north=Math.round(point.y/div);return zeropad(east,p)+' '+zeropad(north,p);}
JSITM.itmRef2Point=function(s){var precision;for(precision=6;precision>=3;precision--){var pattern=new RegExp("^(\\d{"+precision+"})\\s*:?\\s*(\\d{"+precision+"})$","i")
var ref=s.match(pattern);if(ref){if(precision>0){var mult=Math.pow(10,6-precision);var x=parseInt(ref[1],10)*mult;var y=parseInt(ref[2],10)*mult;return new JSITM.Point(x,y);}}}
throw"Could not parse reference";}
JSITM.itm2gps=function(point){var latlng=this.ITM.unproject(point);return latlng.convertGrid(this.GRS80,this.WGS84,this.GRS80toWGS84);}
JSITM.gps2itm=function(latlng){var wgs84=latlng.convertGrid(this.WGS84,this.GRS80,this.WGS84toGRS80);return this.ITM.project(wgs84);}
JSITM.itmRef2gps=function(s){var point=this.itmRef2Point(s);return this.itm2gps(point);}
JSITM.itmRef2gpsRef=function(s){return this.itmRef2gps(s).toString();}
JSITM.gpsRef2itm=function(s){var latlng=this.latlngFromString(s);return this.gps2itm(latlng);}
JSITM.gpsRef2itmRef=function(s,precision){return this.point2ItmRef(this.gpsRef2itm(s),precision||6);}
var ADD_DISCUSSION="צרו דיון";var NEW_FEATURES="עדכן אותי לגבי תכונות חדשות"
var PROVIDERS={};PROVIDERS[1]=PROVIDERS[3]={name:"הלשכה המרכזית לסטטיסטיקה",url:"http://www.cbs.gov.il",logo:"cbs.png",};PROVIDERS[2]={name:"איחוד הצלה",url:"http://www.1221.org.il",logo:"united.jpg",};var PROVIDER_CODE_UNITED_HATZALA=2;var MARKER_TYPE_ACCIDENT=1;var MARKER_TYPE_DISCUSSION=2;var HIGHLIGHT_TYPE_USER_SEARCH=1;var HIGHLIGHT_TYPE_USER_GPS=2;var SEVERITY_FATAL=1;var SEVERITY_SEVERE=2;var SEVERITY_LIGHT=3;var SEVERITY_VARIOUS=4;var SEVERITY_ATTRIBUTES={};SEVERITY_ATTRIBUTES[SEVERITY_FATAL]="showFatal";SEVERITY_ATTRIBUTES[SEVERITY_SEVERE]="showSevere";SEVERITY_ATTRIBUTES[SEVERITY_LIGHT]="showLight";var ACCIDENT_TYPE_CAR_TO_CAR=-1;var ACCIDENT_TYPE_CAR_TO_OBJECT=-2;var ACCIDENT_TYPE_CAR_TO_PEDESTRIAN=1;var ACCIDENT_TYPE_FRONT_TO_SIDE=2;var ACCIDENT_TYPE_FRONT_TO_REAR=3;var ACCIDENT_TYPE_SIDE_TO_SIDE=4;var ACCIDENT_TYPE_FRONT_TO_FRONT=5;var ACCIDENT_TYPE_WITH_STOPPED_CAR_NO_PARKING=6;var ACCIDENT_TYPE_WITH_STOPPED_CAR_PARKING=7;var ACCIDENT_TYPE_WITH_STILL_OBJECT=8;var ACCIDENT_TYPE_OFF_ROAD_OR_SIDEWALK=9;var ACCIDENT_TYPE_ROLLOVER=10;var ACCIDENT_TYPE_SKID=11;var ACCIDENT_TYPE_HIT_PASSSENGER_IN_CAR=12;var ACCIDENT_TYPE_FALLING_OFF_MOVING_VEHICLE=13;var ACCIDENT_TYPE_FIRE=14;var ACCIDENT_TYPE_OTHER=15;var ACCIDENT_TYPE_BACK_TO_FRONT=17;var ACCIDENT_TYPE_BACK_TO_SIDE=18;var ACCIDENT_TYPE_WITH_ANIMAL=19;var ACCIDENT_TYPE_WITH_VEHICLE_LOAD=20;var ACCIDENT_TYPE_UNITED_HATZALA=21;var ICONS={};ICONS[SEVERITY_FATAL]={};ICONS[SEVERITY_SEVERE]={};ICONS[SEVERITY_LIGHT]={};ICONS[SEVERITY_FATAL][ACCIDENT_TYPE_CAR_TO_PEDESTRIAN]="/static/img/icons/vehicle_person_lethal.png";ICONS[SEVERITY_SEVERE][ACCIDENT_TYPE_CAR_TO_PEDESTRIAN]="/static/img/icons/vehicle_person_severe.png";ICONS[SEVERITY_LIGHT][ACCIDENT_TYPE_CAR_TO_PEDESTRIAN]="/static/img/icons/vehicle_person_medium.png";ICONS[SEVERITY_FATAL][ACCIDENT_TYPE_CAR_TO_CAR]="/static/img/icons/vehicle_vehicle_lethal.png";ICONS[SEVERITY_SEVERE][ACCIDENT_TYPE_CAR_TO_CAR]="/static/img/icons/vehicle_vehicle_severe.png";ICONS[SEVERITY_LIGHT][ACCIDENT_TYPE_CAR_TO_CAR]="/static/img/icons/vehicle_vehicle_medium.png";ICONS[SEVERITY_FATAL][ACCIDENT_TYPE_CAR_TO_OBJECT]="/static/img/icons/vehicle_object_lethal.png";ICONS[SEVERITY_SEVERE][ACCIDENT_TYPE_CAR_TO_OBJECT]="/static/img/icons/vehicle_object_severe.png";ICONS[SEVERITY_LIGHT][ACCIDENT_TYPE_CAR_TO_OBJECT]="/static/img/icons/vehicle_object_medium.png";ICONS[SEVERITY_LIGHT][ACCIDENT_TYPE_UNITED_HATZALA]="/static/img/icons/vehicle_unknown_medium.png";ICONS[SEVERITY_SEVERE][ACCIDENT_TYPE_UNITED_HATZALA]="/static/img/icons/vehicle_unknown_severe.png";var MULTIPLE_ICONS={};MULTIPLE_ICONS[SEVERITY_FATAL]="/static/img/icons/multiple_lethal.png";MULTIPLE_ICONS[SEVERITY_SEVERE]="/static/img/icons/multiple_severe.png";MULTIPLE_ICONS[SEVERITY_LIGHT]="/static/img/icons/multiple_medium.png";MULTIPLE_ICONS[SEVERITY_VARIOUS]="/static/img/icons/multiple_various.png";var USER_LOCATION_ICON="/static/img/icons/you_are_Here.png";var DISCUSSION_ICON="/static/img/icons/discussion.png";var ACCIDENT_MINOR_TYPE_TO_TYPE={};ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_CAR_TO_PEDESTRIAN]=ACCIDENT_TYPE_CAR_TO_PEDESTRIAN;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_FRONT_TO_SIDE]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_FRONT_TO_REAR]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_SIDE_TO_SIDE]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_FRONT_TO_FRONT]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_WITH_STOPPED_CAR_NO_PARKING]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_WITH_STOPPED_CAR_PARKING]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_WITH_STILL_OBJECT]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_OFF_ROAD_OR_SIDEWALK]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_ROLLOVER]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_SKID]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_HIT_PASSSENGER_IN_CAR]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_FALLING_OFF_MOVING_VEHICLE]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_FIRE]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_OTHER]=ACCIDENT_TYPE_CAR_TO_OBJECT;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_BACK_TO_FRONT]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_BACK_TO_SIDE]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_WITH_ANIMAL]=ACCIDENT_TYPE_CAR_TO_PEDESTRIAN;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_WITH_VEHICLE_LOAD]=ACCIDENT_TYPE_CAR_TO_CAR;ACCIDENT_MINOR_TYPE_TO_TYPE[ACCIDENT_TYPE_UNITED_HATZALA]=ACCIDENT_TYPE_UNITED_HATZALA;var DEFAULT_ICON=ICONS[1][1];function getIcon(accidentType,severity){var icon=DEFAULT_ICON;try{if(accidentType=="multiple"){icon=MULTIPLE_ICONS[severity];}else{icon=ICONS[severity][ACCIDENT_MINOR_TYPE_TO_TYPE[accidentType]];}}catch(err){}
if(isRetina){var googleIcon={url:icon,scaledSize:new google.maps.Size(30,50)};icon=googleIcon;}
return icon;}
var TYPE_STRING=["","תאונה","הצעה","עצומה"];var TYPES_MAP={};TYPES_MAP['Accident']=TYPE_STRING[1];var SEVERITY_MAP={};SEVERITY_MAP[SEVERITY_FATAL]='קטלנית';SEVERITY_MAP[SEVERITY_SEVERE]='קשה';SEVERITY_MAP[SEVERITY_LIGHT]='קלה';var SUBTYPE_STRING=["פגיעה בהולך רגל","התנגשות חזית אל צד","התנגשות חזית באחור","התנגשות צד בצד","התנגשות חזית אל חזית","התנגשות עם רכב שנעצר ללא חניה","התנגשות עם רכב חונה","התנגשות עם עצם דומם","ירידה מהכביש או עלייה למדרכה","התהפכות","החלקה","פגיעה בנוסע בתוך כלי רכב","נפילה ברכב נע","שריפה","אחר","התנגשות אחור אל חזית","התנגשות אחור אל צד","התנגשות עם בעל חיים","פגיעה ממטען של רכב"];var FILTER_MARKERS=0;var FILTER_INFO=1;var FILTER_DATE=2;var SELECTED_MARKER='(נבחר)';var INACCURATE_MARKER_OPACITY=0.5;var MarkerView=Backbone.View.extend({events:{"click .delete-button":"clickDelete","click .accordion-container input":"accordionInputClick"},initialize:function(options){this.map=options.map;this.markerIconType=options.markerIconType;_.bindAll(this,"clickMarker");},localize:function(field,value){if(this.model.has(value)&&this.model.get(value)!=""&&localization[field][this.model.get(value)]!=undefined){this.$el.find("."+value).text(fields[field]+": "+localization[field][this.model.get(value)]);}},render:function(){var markerPosition=new google.maps.LatLng(this.model.get("latitude"),this.model.get("longitude"));var provider=PROVIDERS[this.model.get("provider_code")];this.marker=new google.maps.Marker({position:markerPosition,id:this.model.get("id"),class:'marker'});if(app.map.zoom<MINIMAL_ZOOM){return this;}
this.marker.setIcon(this.getIcon());this.marker.setMap(this.map);this.marker.view=this;if(this.model.get("type")==MARKER_TYPE_DISCUSSION){this.marker.setTitle(this.getTitle('discussion'));google.maps.event.addListener(this.marker,"click",_.bind(app.showDiscussion,app,this.model.get("identifier")));return this;}
this.marker.setTitle(this.getTitle('single'));if(this.model.get("provider_code")==PROVIDER_CODE_UNITED_HATZALA){app.oms.addMarker(this.marker);this.$el.html($("#united-marker-content-template").html());this.$el.find(".title").text(this.marker.get("title"));this.$el.find(".content").text("תיאור אירוע: "+this.marker.get("title"));this.$el.find(".creation-date").text("תאריך: "+moment(this.model.get("created")).format("LLLL"));this.$el.find(".address").text("מיקום: "+this.model.get("address"));if(this.model.get("description")!=""){this.$el.find(".comments").text("הערות: "+this.model.get("description"));}
this.$el.find(".profile-image").attr("width","70px");this.$el.find(".profile-image").attr("src","/static/img/logos/"+provider.logo);this.$el.find(".profile-image").attr("title",provider.name);this.$el.find(".profile-image-url").attr("href",provider.url);this.$el.find(".added-by").html("מקור: <a href=\'"+
provider.url+"\' target=\'_blank\'>"+provider.name+"</a>");}else{this.$el.html($("#marker-content-template").html());this.$el.find(".title").text(this.marker.get("title"));this.$el.find(".id").text(fields.ACC_ID+": "+this.marker.get("id"));this.$el.find(".provider_code").text(fields.PROVIDER_CODE+": "+this.model.get("provider_code"));this.$el.find(".roadType").text(fields.SUG_DEREH+": "+localization.SUG_DEREH[this.model.get("roadType")]);this.$el.find(".accidentType").text(fields.SUG_TEUNA+": "+localization.SUG_TEUNA[this.model.get("subtype")]);this.$el.find(".roadShape").text(fields.ZURAT_DEREH+": "+localization.ZURAT_DEREH[this.model.get("roadShape")]);this.$el.find(".severityText").text(fields.HUMRAT_TEUNA+": "+localization.HUMRAT_TEUNA[this.model.get("severity")]);this.$el.find(".dayType").text(fields.SUG_YOM+": "+localization.SUG_YOM[this.model.get("dayType")]);this.$el.find(".igun").text(fields.STATUS_IGUN+": "+localization.STATUS_IGUN[this.model.get("locationAccuracy")]);this.$el.find(".unit").text(fields.YEHIDA+": "+localization.YEHIDA[this.model.get("unit")]);this.$el.find(".mainStreet").text(this.model.get("mainStreet"));this.$el.find(".secondaryStreet").text(this.model.get("secondaryStreet"));this.$el.find(".junction").text(this.model.get("junction"));this.localize("HAD_MASLUL","one_lane");this.localize("RAV_MASLUL","multi_lane");this.localize("MEHIRUT_MUTERET","speed_limit");this.localize("TKINUT","intactness");this.localize("ROHAV","road_width");this.localize("SIMUN_TIMRUR","road_sign");this.localize("TEURA","road_light");this.localize("BAKARA","road_control");this.localize("MEZEG_AVIR","weather");this.localize("PNE_KVISH","road_surface");this.localize("SUG_EZEM","road_object");this.localize("MERHAK_EZEM","object_distance");this.localize("LO_HAZA","didnt_cross");this.localize("OFEN_HAZIYA","cross_mode");this.localize("MEKOM_HAZIYA","cross_location");this.localize("KIVUN_HAZIYA","cross_direction");this.$el.find(".creation-date").text("תאריך: "+
moment(this.model.get("created")).format("LLLL"));this.$el.find(".profile-image").attr("width","50px");this.$el.find(".profile-image").attr("src","/static/img/logos/"+provider.logo);this.$el.find(".profile-image").attr("title",provider.name);this.$el.find(".profile-image-url").attr("href",provider.url);this.$el.find(".added-by").html("מקור: <a href=\'"+
provider.url+"\' target=\'_blank\'>"+provider.name+"</a>");}
app.oms.addMarker(this.marker);return this;},getIcon:function(){return{path:google.maps.SymbolPath.CIRCLE,scale:0,fillColor:'black'};},getTitle:function(markerType){var markerTitle='';var accuracy='';var markerIconTypeIdentifier=this.markerIconType?'':'.';switch(markerType){case'single':var loc='';if(this.model.get("junction")!==null){loc=this.model.get("junction");}else{if(this.model.get("secondaryStreet")!==null){loc="ברחוב "+this.model.get("secondaryStreet")+" פינת "+this.model.get("mainStreet");}
else{if(this.model.get("mainStreet")!==null){loc="ברחוב "+this.model.get("mainStreet");}}}
accuracy=!(this.model.get("locationAccuracy")==1);markerTitle="ביום "+moment(this.model.get("created")).format("dddd")+", ה-"
...
+loc;break;case'multiple':accuracy=(this.marker.opacity!=1);markerTitle='מספר תאונות בנקודה זו';break;case'discussion':markerTitle='דיון';break;default:break;}
if(markerType=='discussion')
return markerTitle+markerIconTypeIdentifier;else{accuracy=accuracy?' (מיקום משוער)':'';return markerTitle+accuracy+markerIconTypeIdentifier;}},choose:function(){if(this.model.get("groupID")&&!this.model.get("currentlySpiderfied")){new google.maps.event.trigger(this.marker,"click");}
new google.maps.event.trigger(this.marker,"click");},getUrl:function(){return"/?marker="+this.model.get("id")+"&"+app.getCurrentUrlParams();},localize_data:function(data,field,value,dataType,involved_or_vehicles){switch(dataType){case"invs":if(inv_dict[field]!=undefined&&inv_dict[field][data[i][value]]!=undefined){text_line="<p style=margin:0>"+fields[field]+": "+inv_dict[field][data[i][value]]+"</p>";that.$el.find("#"+involved_or_vehicles).append(text_line);}
break;case"vehs":if(veh_dict[field]!=undefined&&veh_dict[field][data[i][value]]!=undefined){text_line="<p style=margin:0>"+fields[field]+": "+veh_dict[field][data[i][value]]+"</p>";that.$el.find("#"+involved_or_vehicles).append(text_line);}
break;case"nums":if(value=="seats"&&data[i]["seats"]==99){break;};if([data[i][value]]!=undefined&&[data[i][value]]!=0&&[data[i][value]]!=-1){text_line="<p style=margin:0>"+fields[field]+": "+data[i][value]+"</p>";that.$el.find("#"+involved_or_vehicles).append(text_line);}
break;}},clickMarker:function(){that=this;app.closeInfoWindow();app.selectedMarker=this;var infoWindowOffset=this.markerIconType?new google.maps.Size(0,-25):new google.maps.Size(0,0);if(this.marker_clicked){app.infoWindow=new google.maps.InfoWindow({content:that.el,pixelOffset:infoWindowOffset});app.infoWindow.open(that.map,that.marker);app.updateUrl(that.getUrl());}else{this.marker_clicked=true;$.get("/markers/"+this.model.get("id"),function(data){data=JSON.parse(data);var j=1;for(i in data){if(data[i]["sex"]!=undefined){text_line="<p style=margin:0><strong>פרטי אדם מעורב"+" "+(i*1+1)+"</strong></p>";that.$el.find("#involved").append(text_line);that.localize_data(data,"SUG_MEORAV","involved_type","invs","involved");that.localize_data(data,"SHNAT_HOZAA","license_acquiring_date","nums","involved");that.localize_data(data,"KVUZA_GIL","age_group","nums","involved");that.localize_data(data,"MIN","sex","invs","involved");that.localize_data(data,"MAHOZ_MEGURIM","home_district","nums","involved");that.localize_data(data,"SUG_REHEV_NASA_LMS","car_type","invs","involved");that.localize_data(data,"EMZAE_BETIHUT","safety_measures","invs","involved");that.localize_data(data,"HUMRAT_PGIA","injury_severity","invs","involved");that.localize_data(data,"SUG_NIFGA_LMS","injured_type","invs","involved");that.localize_data(data,"PEULAT_NIFGA_LMS","injured_position","invs","involved");that.localize_data(data,"KVUTZAT_OHLUSIYA_LMS","population_type","nums","involved");that.$el.find("#involved").append("<p></p>");}else{text_line="<p style=margin:0><strong>פרטי רכב מעורב"+" "+(j)+"</strong></p>";that.$el.find("#vehicles").append(text_line);that.localize_data(data,"SUG_REHEV_LMS","vehicle_type","vehs","vehicles");that.localize_data(data,"NEFAH","engine_volume","nums","vehicles");that.localize_data(data,"SHNAT_YITZUR","manufacturing_year","nums","vehicles");that.localize_data(data,"KIVUNE_NESIA","driving_directions","nums","vehicles");that.localize_data(data,"MATZAV_REHEV","vehicle_status","vehs","vehicles");that.localize_data(data,"SHIYUH_REHEV_LMS","vehicle_attribution","vehs","vehicles");that.localize_data(data,"MEKOMOT_YESHIVA_LMS","seats","nums","vehicles");that.localize_data(data,"MISHKAL_KOLEL_LMS","total_weight","nums","vehicles");that.$el.find("#vehicles").append("<p></p>");j++;}}
app.infoWindow=new google.maps.InfoWindow({content:that.el,pixelOffset:infoWindowOffset});app.infoWindow.open(that.map,that.marker);app.updateUrl(that.getUrl());});}
$(document).keydown(app.ESCinfoWindow);},clickShare:function(){FB.ui({method:"feed",name:this.model.get("title"),link:document.location.href,description:this.model.get("description"),caption:SUBTYPE_STRING[this.model.get("subtype")]});},accordionInputClick:function(e){var input=e.currentTarget;if(input.checked){var infoWindow=$(input).parents(".gm-style-iw > div");var labelTop=$(input).siblings("label").offset().top;var infoWindowPos=infoWindow.offset().top;var IwHalfHeight=infoWindow.height()/2;var labelPosRelativeToIw=labelTop-infoWindowPos;if(labelPosRelativeToIw>IwHalfHeight){var curIwScroll=infoWindow.scrollTop();var labelPos=labelTop-infoWindowPos+curIwScroll;setTimeout(function(){var sectionHeightScroll=$(input).siblings(".accordion-roller").height()+curIwScroll;var bestScrollTo=Math.min(sectionHeightScroll,labelPos);var scrollAnimationDuration=Math.min((bestScrollTo-curIwScroll)*9,1300);$(infoWindow).on("mousewheel keyup mousedown DOMMouseScroll wheel touchmove",function(){$(infoWindow).stop();});$(infoWindow).animate({scrollTop:bestScrollTo},scrollAnimationDuration,function(){$(infoWindow).off("mousewheel keyup mousedown DOMMouseScroll wheel touchmove");});}.bind(this),550);}}},select:function(){_.each(app.markerList,function(markerView){markerView.unselect();});this.marker.setTitle(this.marker.getTitle()+SELECTED_MARKER);},unselect:function(){if(this.isSelected()){this.marker.setTitle(this.marker.getTitle().replace(SELECTED_MARKER,""));}},isSelected:function(){return this.marker.getTitle().search(SELECTED_MARKER)!=-1;},});var ClusterView=Backbone.View.extend({events:{},initialize:function(options){this.map=options.map;},render:function(){app.clusterer.addCluster(this.model.get("latitude"),this.model.get("longitude"),this.model.get("size"));return this;}});var FeatureDialog=Backbone.View.extend({className:"feature-dialog",events:{"submit":"submit","click .save-button":"submit","click .close-button":"close"},initialize:function(options){this.markers=options.markers;this.type=options.type;this.event=options.event;},render:function(){this.$el.append($("#feature-dialog-template").html());$(document.body).append(this.$el);this.modal=this.$el.find(".modal");this.modal.modal("show");return this;},close:function(){this.modal.modal("hide");},submit:function()
{var adr=this.$el.find("#emailAddress").val();var firstname=this.$el.find("#fName").val();var lastname=this.$el.find("#lName").val();$.post("new-features",JSON.stringify({'address':adr,'fname':firstname,'lname':lastname}));this.close();}});var markerCount=0;var SidebarView=Backbone.View.extend({className:"info-window",events:{"click .current-view li":"clickEntry","mouseover .current-view li":"mouseOverEntry","mouseout .current-view li":"mouseOutEntry",},initialize:function(options){this.map=options.map;this.sidebarItemTemplate=_.template($("#sidebarItemTemplate").text());},render:function(){this.$el.append($("#sidebar-template").html());this.$currentViewList=this.$el.find(".current-view");var self=this;return this;},emptyMarkerList:function(){this.$currentViewList.empty();},reloadMarkerList:function(markerList){markerList=markerList||[];markerCount=0;var bounds=this.map.getBounds();markerList=_.sortBy(markerList,function(marker){return-1*moment(marker.model.get("created")).unix();});var $viewList=$('<ul/>');for(var i=0;i<markerList.length;i++){var markerView=markerList[i];var marker=markerView.marker;if(markerView.isSelected()){markerView.unselect();}
if(bounds.contains(marker.getPosition())){var markerModel=markerView.model;if(markerModel.get("type")==MARKER_TYPE_DISCUSSION){continue;}
var iconUrl=markerView.getIcon();var entryHtml=this.sidebarItemTemplate({created:moment(markerModel.get("created")).format("LLLL"),type:localization.SUG_TEUNA[markerModel.get("subtype")],icon:markerView.getTitle("single"),});var $entry=$(entryHtml);$entry.data("marker",marker);$viewList.append($entry);markerCount++;}}
this.$currentViewList.empty().append($viewList.find("li"));this.$el.find(".current-view-count").text(markerCount);app.updateFilterString();},updateLayers:function(){this.$el.find("img.checkbox-severity").each(function(){attr=SEVERITY_ATTRIBUTES[parseInt($(this).data("type"))];app.model.set(attr,$(this).data("checked"));});},clickEntry:function(e){var marker=this.getMarker(e);if(marker){marker.view.choose();}},getMarker:function(e){return $(e.target).data("marker")||$(e.target).parents("li").data("marker");},setResponsively:function(){var filterPanelTop=this.$el.find(".filter-description").outerHeight()+this.$el.find(".filter-menu").outerHeight();var filterPanel=this.$el.find(".filter-panel");$(filterPanel).css("top",filterPanelTop);var currentViewTop=this.$el.find("#step6tour .btn-label").outerHeight()+this.$el.find("#step6tour .btn-wrap").outerHeight();var currentView=this.$el.find(".current-view");$(currentView).css("top",currentViewTop);},mouseOverEntry:function(e){var markerOfEntry=this.getMarker(e);if(markerOfEntry){markerOfEntry.view.select();}},mouseOutEntry:function(e){var markerOfEntry=this.getMarker(e);if(markerOfEntry){markerOfEntry.view.unselect();}},});window.ContextMenuView=Backbone.View.extend({tagName:"ul",className:"context-menu",initialize:function(options){this.items=options.items;},events:{"click li":"clickItem"},render:function(e){for(var i=0;i<this.items.length;i++){$('<li data-toggle="modal" href="'+this.items[i].href+'" class="btn">').appendTo(this.$el).append($("<span/>",{class:"icon-"+this.items[i].icon})).append($("<span/>",{class:"text",text:" "+this.items[i].text}));}
$(document.body).append(this.$el);this.event=e;this.$el.css({top:e.pixel.y+40,left:e.pixel.x});$(document.body).one("contextmenu click",_.bind(this.remove,this));return this;},remove:function(){this.$el.remove();},clickItem:function(e){this.items[$(e.currentTarget).index()].callback();}});var MAP_STYLE=[{"featureType":"water","elementType":"water","stylers":[{"visibility":"on"},{"color":"#b4c6cd"}]},{"featureType":"landscape.natural","elementType":"landscape.natural","stylers":[{"color":"#f2e8de"}]},{"featureType":"landscape.man_made","elementType":"landscape.man_made","stylers":[{"color":"#e9e5db"}]},{"featureType":"poi","elementType":"poi","stylers":[{"visibility":"off"}]},{"featureType":"poi.park","elementType":"geometry","stylers":[{"visibility":"on"},{"color":"#cbdfa3"}]},{"featureType":"road.highway","elementType":"geometry.fill","stylers":[{"color":"#ffffff"}]},{"featureType":"road.arterial","elementType":"geometry","stylers":[{"color":"#ffffff"}]},{"featureType":"road.local","elementType":"geometry.fill","stylers":[{"color":"#ffffff"}]},{"featureType":"road.highway","elementType":"geometry.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.arterial","elementType":"geometry.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.highway","elementType":"geometry.stroke","stylers":[{"visibility":"off"}]},{"featureType":"landscape","elementType":"labels.text","stylers":[{"color":"#505050"},{"visibility":"off"}]},{"featureType":"transit.line","elementType":"geometry.fill","stylers":[{"color":"#969696"}]}]
$(document).ready(function(){var url=document.URL,$map_link=$("#map_link"),$iframe_link=$("#iframe_link"),$embed_link=$("#js-embed-link");$embed_link.on("click",function(){url=document.URL;$map_link.val(url);$iframe_link.html('<iframe src="'+url+'&map_only=true"></iframe>');});$(".js-btn-copytoclipboard").on("click",function(){$("#"+$(this).data("copy")).select();});});!function(t,e){var o,n;return n=e.document,o=function(){function o(o){var n;try{n=e.localStorage}catch(i){n=!1}this._options=t.extend({name:"tour",steps:[],container:"body",autoscroll:!0,keyboard:!0,storage:n,debug:!1,backdrop:!1,backdropPadding:0,redirect:!0,orphan:!1,duration:!1,delay:!1,basePath:"",template:'<div class="popover" role="tooltip"> <div class="arrow"></div> <h3 class="popover-title"></h3> <div class="popover-content"></div> <div class="popover-navigation"> <div class="btn-group"> <button class="btn btn-sm btn-default" data-role="prev">&laquo; Prev</button> <button class="btn btn-sm btn-default" data-role="next">Next &raquo;</button> <button class="btn btn-sm btn-default" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume">Pause</button> </div> <button class="btn btn-sm btn-default" data-role="end">End tour</button> </div> </div>',afterSetState:function(){},afterGetState:function(){},afterRemoveState:function(){},onStart:function(){},onEnd:function(){},onShow:function(){},onShown:function(){},onHide:function(){},onHidden:function(){},onNext:function(){},onPrev:function(){},onPause:function(){},onResume:function(){}},o),this._force=!1,this._inited=!1,this.backdrop={overlay:null,$element:null,$background:null,backgroundShown:!1,overlayElementShown:!1}}return o.prototype.addSteps=function(t){var e,o,n;for(o=0,n=t.length;n>o;o++)e=t[o],this.addStep(e);return this},o.prototype.addStep=function(t){return this._options.steps.push(t),this},o.prototype.getStep=function(e){return null!=this._options.steps[e]?t.extend({id:"step-"+e,path:"",placement:"right",title:"",content:"<p></p>",next:e===this._options.steps.length-1?-1:e+1,prev:e-1,animation:!0,container:this._options.container,autoscroll:this._options.autoscroll,backdrop:this._options.backdrop,backdropPadding:this._options.backdropPadding,redirect:this._options.redirect,orphan:this._options.orphan,duration:this._options.duration,delay:this._options.delay,template:this._options.template,onShow:this._options.onShow,onShown:this._options.onShown,onHide:this._options.onHide,onHidden:this._options.onHidden,onNext:this._options.onNext,onPrev:this._options.onPrev,onPause:this._options.onPause,onResume:this._options.onResume},this._options.steps[e]):void 0},o.prototype.init=function(t){return this._force=t,this.ended()?(this._debug("Tour ended, init prevented."),this):(this.setCurrentStep(),this._initMouseNavigation(),this._initKeyboardNavigation(),this._onResize(function(t){return function(){return t.showStep(t._current)}}(this)),null!==this._current&&this.showStep(this._current),this._inited=!0,this)},o.prototype.start=function(t){var e;return null==t&&(t=!1),this._inited||this.init(t),null===this._current&&(e=this._makePromise(null!=this._options.onStart?this._options.onStart(this):void 0),this._callOnPromiseDone(e,this.showStep,0)),this},o.prototype.next=function(){var t;return t=this.hideStep(this._current),this._callOnPromiseDone(t,this._showNextStep)},o.prototype.prev=function(){var t;return t=this.hideStep(this._current),this._callOnPromiseDone(t,this._showPrevStep)},o.prototype.goTo=function(t){var e;return e=this.hideStep(this._current),this._callOnPromiseDone(e,this.showStep,t)},o.prototype.end=function(){var o,i;return o=function(o){return function(){return t(n).off("click.tour-"+o._options.name),t(n).off("keyup.tour-"+o._options.name),t(e).off("resize.tour-"+o._options.name),o._setState("end","yes"),o._inited=!1,o._force=!1,o._clearTimer(),null!=o._options.onEnd?o._options.onEnd(o):void 0}}(this),i=this.hideStep(this._current),this._callOnPromiseDone(i,o)},o.prototype.ended=function(){return!this._force&&!!this._getState("end")},o.prototype.restart=function(){return this._removeState("current_step"),this._removeState("end"),this.start()},o.prototype.pause=function(){var t;return t=this.getStep(this._current),t&&t.duration?(this._paused=!0,this._duration-=(new Date).getTime()-this._start,e.clearTimeout(this._timer),this._debug("Paused/Stopped step "+(this._current+1)+" timer ("+this._duration+" remaining)."),null!=t.onPause?t.onPause(this,this._duration):void 0):this},o.prototype.resume=function(){var t;return t=this.getStep(this._current),t&&t.duration?(this._paused=!1,this._start=(new Date).getTime(),this._duration=this._duration||t.duration,this._timer=e.setTimeout(function(t){return function(){return t._isLast()?t.next():t.end()}}(this),this._duration),this._debug("Started step "+(this._current+1)+" timer with duration "+this._duration),null!=t.onResume&&this._duration!==t.duration?t.onResume(this,this._duration):void 0):this},o.prototype.hideStep=function(e){var o,n,i;return(i=this.getStep(e))?(this._clearTimer(),n=this._makePromise(null!=i.onHide?i.onHide(this,e):void 0),o=function(o){return function(){var n;return n=t(i.element),n.data("bs.popover")||n.data("popover")||(n=t("body")),n.popover("destroy").removeClass("tour-"+o._options.name+"-element tour-"+o._options.name+"-"+e+"-element"),i.reflex&&n.removeClass("tour-step-element-reflex").off(""+o._reflexEvent(i.reflex)+".tour-"+o._options.name),i.backdrop&&o._hideBackdrop(),null!=i.onHidden?i.onHidden(o):void 0}}(this),this._callOnPromiseDone(n,o),n):void 0},o.prototype.showStep=function(t){var o,i,r,s;return this.ended()?(this._debug("Tour ended, showStep prevented."),this):(s=this.getStep(t))?(r=t<this._current,o=this._makePromise(null!=s.onShow?s.onShow(this,t):void 0),i=function(e){return function(){var o,i,a;if(e.setCurrentStep(t),i=function(){switch({}.toString.call(s.path)){case"[object Function]":return s.path();case"[object String]":return this._options.basePath+s.path;default:return s.path}}.call(e),o=[n.location.pathname,n.location.hash].join(""),e._isRedirect(i,o))return e._redirect(s,i),void 0;if(e._isOrphan(s)){if(!s.orphan)return e._debug("Skip the orphan step "+(e._current+1)+".\nOrphan option is false and the element does not exist or is hidden."),r?e._showPrevStep():e._showNextStep(),void 0;e._debug("Show the orphan step "+(e._current+1)+". Orphans option is true.")}return s.backdrop&&e._showBackdrop(e._isOrphan(s)?void 0:s.element),a=function(){return e.getCurrentStep()===t?(null!=s.element&&s.backdrop&&e._showOverlayElement(s),e._showPopover(s,t),null!=s.onShown&&s.onShown(e),e._debug("Step "+(e._current+1)+" of "+e._options.steps.length)):void 0},s.autoscroll?e._scrollIntoView(s.element,a):a(),s.duration?e.resume():void 0}}(this),s.delay?(this._debug("Wait "+s.delay+" milliseconds to show the step "+(this._current+1)),e.setTimeout(function(t){return function(){return t._callOnPromiseDone(o,i)}}(this),s.delay)):this._callOnPromiseDone(o,i),o):void 0},o.prototype.getCurrentStep=function(){return this._current},o.prototype.setCurrentStep=function(t){return null!=t?(this._current=t,this._setState("current_step",t)):(this._current=this._getState("current_step"),this._current=null===this._current?null:parseInt(this._current,10)),this},o.prototype._setState=function(t,e){var o,n;if(this._options.storage){n=""+this._options.name+"_"+t;try{this._options.storage.setItem(n,e)}catch(i){o=i,o.code===DOMException.QUOTA_EXCEEDED_ERR&&this.debug("LocalStorage quota exceeded. State storage failed.")}return this._options.afterSetState(n,e)}return null==this._state&&(this._state={}),this._state[t]=e},o.prototype._removeState=function(t){var e;return this._options.storage?(e=""+this._options.name+"_"+t,this._options.storage.removeItem(e),this._options.afterRemoveState(e)):null!=this._state?delete this._state[t]:void 0},o.prototype._getState=function(t){var e,o;return this._options.storage?(e=""+this._options.name+"_"+t,o=this._options.storage.getItem(e)):null!=this._state&&(o=this._state[t]),(void 0===o||"null"===o)&&(o=null),this._options.afterGetState(t,o),o},o.prototype._showNextStep=function(){var t,e,o;return o=this.getStep(this._current),e=function(t){return function(){return t.showStep(o.next)}}(this),t=this._makePromise(null!=o.onNext?o.onNext(this):void 0),this._callOnPromiseDone(t,e)},o.prototype._showPrevStep=function(){var t,e,o;return o=this.getStep(this._current),e=function(t){return function(){return t.showStep(o.prev)}}(this),t=this._makePromise(null!=o.onPrev?o.onPrev(this):void 0),this._callOnPromiseDone(t,e)},o.prototype._debug=function(t){return this._options.debug?e.console.log("Bootstrap Tour '"+this._options.name+"' | "+t):void 0},o.prototype._isRedirect=function(t,e){return null!=t&&""!==t&&("[object RegExp]"==={}.toString.call(t)&&!t.test(e)||"[object String]"==={}.toString.call(t)&&t.replace(/\?.*$/,"").replace(/\/?$/,"")!==e.replace(/\/?$/,""))},o.prototype._redirect=function(e,o){return t.isFunction(e.redirect)?e.redirect.call(this,o):e.redirect===!0?(this._debug("Redirect to "+o),n.location.href=o):void 0},o.prototype._isOrphan=function(e){return null==e.element||!t(e.element).length||t(e.element).is(":hidden")&&"http://www.w3.org/2000/svg"!==t(e.element)[0].namespaceURI},o.prototype._isLast=function(){return this._current<this._options.steps.length-1},o.prototype._showPopover=function(e,o){var n,i,r,s;return t(".tour-"+this._options.name).remove(),s=t.extend({},this._options),r=this._isOrphan(e),e.template=this._template(e,o),r&&(e.element="body",e.placement="top"),n=t(e.element),n.addClass("tour-"+this._options.name+"-element tour-"+this._options.name+"-"+o+"-element"),e.options&&t.extend(s,e.options),e.reflex&&!r&&(n.addClass("tour-step-element-reflex"),n.off(""+this._reflexEvent(e.reflex)+".tour-"+this._options.name),n.on(""+this._reflexEvent(e.reflex)+".tour-"+this._options.name,function(t){return function(){return t._isLast()?t.next():t.end()}}(this))),n.popover({placement:e.placement,trigger:"manual",title:e.title,content:e.content,html:!0,animation:e.animation,container:e.container,template:e.template,selector:e.element}).popover("show"),i=n.data("bs.popover")?n.data("bs.popover").tip():n.data("popover").tip(),i.attr("id",e.id),this._reposition(i,e),r?this._center(i):void 0},o.prototype._template=function(e,o){var n,i,r,s;return s=t.isFunction(e.template)?t(e.template(o,e)):t(e.template),n=s.find(".popover-navigation"),r=n.find('[data-role="prev"]'),i=n.find('[data-role="next"]'),this._isOrphan(e)&&s.addClass("orphan"),s.addClass("tour-"+this._options.name+" tour-"+this._options.name+"-"+o),e.prev<0&&n.find('[data-role="prev"]').addClass("disabled"),e.next<0&&n.find('[data-role="next"]').addClass("disabled"),e.duration||n.find('[data-role="pause-resume"]').remove(),s.clone().wrap("<div>").parent().html()},o.prototype._reflexEvent=function(t){return"[object Boolean]"==={}.toString.call(t)?"click":t},o.prototype._reposition=function(e,o){var i,r,s,a,p,u,h;if(a=e[0].offsetWidth,r=e[0].offsetHeight,h=e.offset(),p=h.left,u=h.top,i=t(n).outerHeight()-h.top-e.outerHeight(),0>i&&(h.top=h.top+i),s=t("html").outerWidth()-h.left-e.outerWidth(),0>s&&(h.left=h.left+s),h.top<0&&(h.top=0),h.left<0&&(h.left=0),e.offset(h),"bottom"===o.placement||"top"===o.placement){if(p!==h.left)return this._replaceArrow(e,2*(h.left-p),a,"left")}else if(u!==h.top)return this._replaceArrow(e,2*(h.top-u),r,"top")},o.prototype._center=function(o){return o.css("top",t(e).outerHeight()/2-o.outerHeight()/2)},o.prototype._replaceArrow=function(t,e,o,n){return t.find(".arrow").css(n,e?50*(1-e/o)+"%":"")},o.prototype._scrollIntoView=function(o,n){var i,r,s,a,p,u;return i=t(o),i.length?(r=t(e),a=i.offset().top,u=r.height(),p=Math.max(0,a-u/2),this._debug("Scroll into view. ScrollTop: "+p+". Element offset: "+a+". Window height: "+u+"."),s=0,t("body, html").stop(!0,!0).animate({scrollTop:Math.ceil(p)},function(t){return function(){return 2===++s?(n(),t._debug("Scroll into view.\nAnimation end element offset: "+i.offset().top+".\nWindow height: "+r.height()+".")):void 0}}(this))):n()},o.prototype._onResize=function(o,n){return t(e).on("resize.tour-"+this._options.name,function(){return clearTimeout(n),n=setTimeout(o,100)})},o.prototype._initMouseNavigation=function(){var e;return e=this,t(n).off("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='prev']").off("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='next']").off("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='end']").off("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='pause-resume']").on("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='next']",function(t){return function(e){return e.preventDefault(),t.next()}}(this)).on("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='prev']",function(t){return function(e){return e.preventDefault(),t.prev()}}(this)).on("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='end']",function(t){return function(e){return e.preventDefault(),t.end()}}(this)).on("click.tour-"+this._options.name,".popover.tour-"+this._options.name+" *[data-role='pause-resume']",function(o){var n;return o.preventDefault(),n=t(this),n.text(e._paused?n.data("pause-text"):n.data("resume-text")),e._paused?e.resume():e.pause()})},o.prototype._initKeyboardNavigation=function(){return this._options.keyboard?t(n).on("keyup.tour-"+this._options.name,function(t){return function(e){if(e.which)switch(e.which){case 39:return e.preventDefault(),t._isLast()?t.next():t.end();case 37:if(e.preventDefault(),t._current>0)return t.prev();break;case 27:return e.preventDefault(),t.end()}}}(this)):void 0},o.prototype._makePromise=function(e){return e&&t.isFunction(e.then)?e:null},o.prototype._callOnPromiseDone=function(t,e,o){return t?t.then(function(t){return function(){return e.call(t,o)}}(this)):e.call(this,o)},o.prototype._showBackdrop=function(){return this.backdrop.backgroundShown?void 0:(this.backdrop=t("<div>",{"class":"tour-backdrop"}),this.backdrop.backgroundShown=!0,t("body").append(this.backdrop))},o.prototype._hideBackdrop=function(){return this._hideOverlayElement(),this._hideBackground()},o.prototype._hideBackground=function(){return this.backdrop?(this.backdrop.remove(),this.backdrop.overlay=null,this.backdrop.backgroundShown=!1):void 0},o.prototype._showOverlayElement=function(e){var o,n;return o=t(e.element),o&&0!==o.length&&!this.backdrop.overlayElementShown?(this.backdrop.overlayElementShown=!0,this.backdrop.$element=o.addClass("tour-step-backdrop"),this.backdrop.$background=t("<div>",{"class":"tour-step-background"}),n={width:o.innerWidth(),height:o.innerHeight(),offset:o.offset()},this.backdrop.$background.appendTo("body"),e.backdropPadding&&(n=this._applyBackdropPadding(e.backdropPadding,n)),this.backdrop.$background.width(n.width).height(n.height).offset(n.offset)):void 0},o.prototype._hideOverlayElement=function(){return this.backdrop.overlayElementShown?(this.backdrop.$element.removeClass("tour-step-backdrop"),this.backdrop.$background.remove(),this.backdrop.$element=null,this.backdrop.$background=null,this.backdrop.overlayElementShown=!1):void 0},o.prototype._applyBackdropPadding=function(t,e){return"object"==typeof t?(null==t.top&&(t.top=0),null==t.right&&(t.right=0),null==t.bottom&&(t.bottom=0),null==t.left&&(t.left=0),e.offset.top=e.offset.top-t.top,e.offset.left=e.offset.left-t.left,e.width=e.width+t.left+t.right,e.height=e.height+t.top+t.bottom):(e.offset.top=e.offset.top-t,e.offset.left=e.offset.left-t,e.width=e.width+2*t,e.height=e.height+2*t),e},o.prototype._clearTimer=function(){return e.clearTimeout(this._timer),this._timer=null,this._duration=null},o}(),e.Tour=o}(jQuery,window);$(function(){var AppRouter=Backbone.Router.extend({routes:{"":"navigateEmpty","/?marker=:id&start_date=:start&end_date=:end&show_fatal=:showFatal&show_severe=:showSevere&show_light=:showLight&show_inaccurate=:showInaccurate&zoom=:zoom&lat=:lat&lon=:lon":"navigate"},navigateEmpty:function(){app.model.set("currentMarker",null);}});var Discussion=Backbone.Model.extend({});window.MarkerCollection=Backbone.Collection.extend({url:"/markers",parse:function(response,options){return response.markers;}});window.ClusterCollection=Backbone.Collection.extend({url:"/clusters",parse:function(response,options){return response.clusters;}});window.AppView=Backbone.View.extend({el:$("#app"),events:{},initialize:function(){this.markers=new MarkerCollection();this.clusters=new ClusterCollection();this.model=new Backbone.Model();this.markerIconType=true;this.markerList=[];this.clusterList=[];this.firstLoadDelay=true;this.show_markers='1';this.show_discussions='1';this.accurate='1';this.approx='1';this.show_fatal='1';this.show_severe='1';this.show_light='1';this.show_urban=3;this.show_intersection=3;this.show_lane=3;this.show_day=7;this.show_holiday=0;this.show_time=24;this.start_time=25;this.end_time=25;this.weather=0;this.road=0;this.separation=0;this.surface=0;this.acctype=0;this.controlmeasure=0;this.district=0;this.case_type=0;this.dateRanges=[new Date($("#sdate").val()),new Date($("#edate").val())];setTimeout(function(){this.firstLoadDelay=false;}.bind(this),2200);this.markers.bind("reset",this.loadMarkers,this).bind("destroy",this.loadMarkers,this).bind("add",this.loadMarker,this).bind("change:currentModel",this.chooseMarker,this);this.clusters.bind("reset",this.loadClusters,this).bind("add",this.loadCluster,this);this.initLayers();this.model.bind("change:user",this.updateUser,this).bind("change:showFatal",_.bind(this.reloadMarkersIfNeeded,this,"showFatal")).bind("change:showSevere",_.bind(this.reloadMarkersIfNeeded,this,"showSevere")).bind("change:showLight",_.bind(this.reloadMarkersIfNeeded,this,"showLight")).bind("change:showInaccurateMarkers",_.bind(this.reloadMarkersIfNeeded,this,"showInaccurateMarkers")).bind("change:dateRange",this.reloadMarkers,this);},reloadMarkersIfNeeded:function(attr){if(this.clusterMode()||this.model.get(attr)){this.reloadMarkers();}else{this.updateUrl();this.loadMarkers();}},updateUrl:function(url){if(typeof url=='undefined'){if(app.infoWindow||app.discussionShown)return;url="";}else{var questionMarkPlace=url.indexOf('?');if(questionMarkPlace!=-1){url=url.substring(0,questionMarkPlace);}}
Backbone.history.navigate(Backbone.history.fragment,false);window.history.pushState('','','/')},clusterMode:function(){return this.map.zoom<MINIMAL_ZOOM;},zoomChanged:function(){this.resetOnMouseUp=true;this.fetchMarkers();},reloadMarkers:function(){if(!this.firstLoadDelay){this.oms.unspiderfy();this.clearMarkersFromMap();this.fetchMarkers();}},fetchMarkers:function(){if(!this.isReady)return;var params=this.buildMarkersParams();if(!params)return;var reset=this.clusterMode()||this.previousZoom<MINIMAL_ZOOM;this.previousZoom=this.map.zoom;if(reset){this.resetMarkers();}
if(this.clusterMode()){this.closeInfoWindow();this.clusters.fetch({data:$.param(params),reset:reset,success:this.reloadSidebar.bind(this)});}else{this.clearClustersFromMap();$("#view-filter").prop('disabled',false);if(!this.markerList.length){this.loadMarkers();}
this.clearClustersFromMap();this.markers.fetch({data:$.param(params),reset:reset,success:this.reloadSidebar.bind(this)});}},reloadSidebar:function(){if(this.clusterMode()){this.sidebar.emptyMarkerList();}else{this.setMultipleMarkersIcon();this.sidebar.reloadMarkerList(this.markerList);}
if(jsPanelInst!=null){startJSPanelWithChart(jsPanelInst,$("#statPanel").width(),$("#statPanel").height(),$("#statPanel").width()-30,$("#statPanel").height()-80);}
this.updateFilterString();this.chooseMarker();if(this.iconTypeChanged==true){this.$el.find(".current-view").toggleClass("sidebar-pin");this.$el.find(".current-view").toggleClass("sidebar-dot");this.iconTypeChanged=false;}},buildMarkersParams:function(isForUrl){var bounds=this.map.getBounds();if(!bounds)return null;var zoom=this.map.zoom;var dateRange=this.model.get("dateRange");var params={};if(typeof isForUrl==='undefined'){params["ne_lat"]=bounds.getNorthEast().lat();params["ne_lng"]=bounds.getNorthEast().lng();params["sw_lat"]=bounds.getSouthWest().lat();params["sw_lng"]=bounds.getSouthWest().lng();params["zoom"]=zoom;params["thin_markers"]=(zoom<MINIMAL_ZOOM||!bounds);params["start_date"]=this.dateRanges[0].getTime()/1000;params["end_date"]=this.dateRanges[1].getTime()/1000;}else{var center=app.map.getCenter();params["zoom"]=zoom;params["start_date"]=moment(this.dateRanges[0]).format("YYYY-MM-DD");params["end_date"]=moment(this.dateRanges[1]).format("YYYY-MM-DD");params["lat"]=center.lat();params["lon"]=center.lng();}
params["show_fatal"]=this.show_fatal;params["show_severe"]=this.show_severe;params["show_light"]=this.show_light;params["approx"]=this.approx;params["accurate"]=this.accurate;params["show_markers"]=this.show_markers;params["show_discussions"]=this.show_discussions;params["show_urban"]=this.show_urban;params["show_intersection"]=this.show_intersection;params["show_lane"]=this.show_lane;params["show_day"]=this.show_day;params["show_holiday"]=this.show_holiday;params["show_time"]=this.show_time;params["start_time"]=this.start_time;params["end_time"]=this.end_time;params["weather"]=this.weather;params["road"]=this.road;params["separation"]=this.separation;params["surface"]=this.surface;params["acctype"]=this.acctype;params["controlmeasure"]=this.controlmeasure;params["district"]=this.district;params["case_type"]=this.case_type;return params;},setMultipleMarkersIcon:function(){var groupID=1;var groupsData=[];_.each(this.oms.markersNearAnyOtherMarker(),function(marker){marker.view.model.unset("groupID");});_.each(this.markerList,function(markerView){var marker=markerView.marker;if(this.map.getBounds().contains(marker.getPosition())){if(!this.oms.markersNearMarker(marker,true).length){marker.setTitle(markerView.getTitle("single"));}}},this);_.each(this.oms.markersNearAnyOtherMarker(),function(marker){if(marker.view.model.get("currentlySpiderfied")){marker.setTitle(marker.view.getTitle("single"));}else{marker.setTitle(marker.view.getTitle("multiple"));}
var firstMember=marker.view.model;if(!firstMember.get("groupID")){firstMember.set("groupID",groupID);var firstMemberSeverity=firstMember.get('severity');var firstMemberOpacity=firstMember.get("locationAccuracy")==1?'opaque':1;groupsData.push({severity:firstMemberSeverity,opacity:firstMemberOpacity});_.each(this.oms.markersNearMarker(marker),function(markerNear){var markerNearModel=markerNear.view.model;markerNearModel.set("groupID",groupID);if((firstMemberSeverity!=markerNearModel.get('severity'))){groupsData[groupsData.length-1].severity=SEVERITY_VARIOUS;}
if(groupsData[groupsData.length-1].opacity!='opaque'){if(markerNearModel.get("locationAccuracy")==1){groupsData[groupsData.length-1].opacity='opaque';}else{groupsData[groupsData.length-1].opacity++;}}});groupID++;}},this);this.groupsData=groupsData;if(tourLocation==5){var myLatlng=new google.maps.LatLng(32.09170,34.86435);var location1=new google.maps.Marker({position:myLatlng,map:this.map,icon:app.retinaIconsResize(MULTIPLE_ICONS[SEVERITY_VARIOUS])});tourLocation=6;console.log("inside the group id "+tourLocation+"new2");contentString='<p>בנקודה זו התרחשו מספר תאונות, לחיצה על האייקון תציג אותן בנפרד ותאפשר בחירה</br> בתאונה בודדת.</p>';titleString='אייקון של מספר תאונות באותו מקום';defInfoWindows();infowindow=new google.maps.InfoWindow({content:htmlTourString,maxWidth:350});infowindow.open(this.map,location1);tourStyle(infowindow);}},downloadCsv:function(){if(this.markers.length>0){params=this.buildMarkersParams();params["format"]="csv";window.location=this.markers.url+"?"+$.param(params);}else{$('#empty-csv-dialog').modal('show');}},linkMap:function(){$('#embed').modal('show');},render:function(){this.isReady=false;this.defaultLocation=new google.maps.LatLng(INIT_LAT,INIT_LON);var mapOptions={center:this.defaultLocation,zoom:INIT_ZOOM,mapTypeId:google.maps.MapTypeId.ROADMAP,mapTypeControl:false,zoomControl:!MAP_ONLY,panControl:!MAP_ONLY,streetViewControl:!MAP_ONLY,styles:MAP_STYLE};this.map=new google.maps.Map(this.$el.find("#map_canvas").get(0),mapOptions);var mapControlDiv=document.createElement('div');mapControlDiv.className="map-control";mapControlDiv.innerHTML=$("#map-control").html();var resetMapDiv=document.createElement('div');resetMapDiv.className="map-button reset-map-control";resetMapDiv.innerHTML=$("#reset-map-control").html();google.maps.event.addDomListener(resetMapDiv,'click',function(){this.goToMyLocation();}.bind(this));var downloadCsvDiv=document.createElement('div');downloadCsvDiv.className="map-button download-csv-control";downloadCsvDiv.innerHTML=$("#download-csv-control").html();google.maps.event.addDomListener(downloadCsvDiv,'click',function(){this.downloadCsv();}.bind(this));var linkMapDiv=document.createElement('div');linkMapDiv.className='map-button link-map-control';linkMapDiv.innerHTML=$("#link-map-control").html();google.maps.event.addDomListener(linkMapDiv,'click',function(){var url=document.URL+"?"+this.getCurrentUrlParams();$map_link=$("#map_link"),$iframe_link=$("#iframe_link"),$embed_link=$("#js-embed-link");$map_link.val(url);$iframe_link.html('<iframe src="'+url+'&map_only=true"></iframe>');$(".js-btn-copytoclipboard").on("click",function(){$("#"+$(this).data("copy")).select();});this.linkMap();}.bind(this));var tourDiv=document.createElement('div');tourDiv.className="map-button tour-control blink";tourDiv.innerHTML=$("#tour-control").html();google.maps.event.addDomListener(tourDiv,'click',function(){tourClick();}.bind(this));var statDiv=document.createElement('div');statDiv.className="map-button statistics-control";statDiv.innerHTML=$("#statistics-control").html();google.maps.event.addDomListener(statDiv,'click',function(){statPanelClick(700,400,700,350);}.bind(this));mapControlDiv.appendChild(resetMapDiv);mapControlDiv.appendChild(downloadCsvDiv);mapControlDiv.appendChild(linkMapDiv);mapControlDiv.appendChild(tourDiv);mapControlDiv.appendChild(statDiv);var linkLabel=document.createElement('div');linkLabel.className='control-label';linkLabel.innerHTML='קישור לתצוגה נוכחית';linkMapDiv.appendChild(linkLabel);var downloadLabel=document.createElement('div');downloadLabel.className='control-label';downloadLabel.innerHTML='הורד נתוני תאונות (CSV)';downloadCsvDiv.appendChild(downloadLabel);var tourLabel=document.createElement('div');tourLabel.className='control-label';tourLabel.innerHTML='התחל הדרכה';tourDiv.appendChild(tourLabel);var statLabel=document.createElement('div');statLabel.className='control-label';statLabel.innerHTML='גרפים';statDiv.appendChild(statLabel);this.map.controls[google.maps.ControlPosition.TOP_RIGHT].push(mapControlDiv);if(LOCATION_SPECIFIED){if(!MARKER_ID&&!DISCUSSION_IDENTIFIER){this.setCenterWithMarker(this.defaultLocation);}}else{this.goToMyLocation();}
setTimeout(function(){this.fetchMarkers();}.bind(this),2000);var input=document.getElementById('pac-input');this.map.controls[google.maps.ControlPosition.TOP_RIGHT].push(input);this.searchBox=new google.maps.places.SearchBox(input);var toggleBGDiv=document.createElement('div');toggleBGDiv.className="toggle-control-bg";toggleBGDiv.innerHTML=$("#toggle-control").html();var toggleDiv=document.createElement('div');toggleDiv.className="map-button toggle-control pin";toggleDiv.title='שנה תצוגת אייקונים';google.maps.event.addDomListener(toggleBGDiv,'click',function(){$(toggleDiv).toggleClass('pin');$(toggleDiv).toggleClass('dot');this.iconTypeChanged=true;this.toggleMarkerIconType();}.bind(this));toggleBGDiv.appendChild(toggleDiv);this.map.controls[google.maps.ControlPosition.TOP_RIGHT].push(toggleBGDiv);google.maps.event.addListener(this.searchBox,'places_changed',function(){this.handleSearchBox();}.bind(this));this.oms=new OverlappingMarkerSpiderfier(this.map,{markersWontMove:true,markersWontHide:true,keepSpiderfied:true});this.oms.addListener("click",function(marker,event){marker.view.clickMarker();this.clickedMarker=true;}.bind(this));this.oms.addListener("spiderfy",function(markers){this.closeInfoWindow();_.each(markers,function(marker){marker.setTitle(marker.view.getTitle('single'));marker.view.model.set("currentlySpiderfied",true);});this.clickedMarker=true;}.bind(this));this.oms.addListener("unspiderfy",function(markers){_.each(markers,function(marker){marker.view.model.unset("currentlySpiderfied");});}.bind(this));this.oms.addListener("unspiderfy",this.setMultipleMarkersIcon.bind(this));console.log('Loaded OverlappingMarkerSpiderfier');var clusterStyle=[{size:42,width:5},{size:52,width:10},{size:62,width:15},{size:72,width:20}];var mcOptions={maxZoom:MINIMAL_ZOOM-1,minimumClusterSize:1,styles:clusterStyle};this.clusterer=new MarkerClusterer(this.map,[],mcOptions);console.log('Loaded MarkerClusterer');this.sidebar=new SidebarView({map:this.map}).render();this.$el.find(".sidebar-container").append(this.sidebar.$el);console.log('Loaded SidebarView');$(document).ajaxStart(function(){this.spinner=$('<li/>');this.spinner.height('20px');this.sidebar.$currentViewList.prepend(this.spinner);this.spinner.spin();}.bind(this));$(document).ajaxStop(function(){if(this.spinner){this.spinner.spin(false);}}.bind(this));console.log('Loaded spinner');this.previousZoom=this.map.zoom;this.router=new AppRouter();Backbone.history.start({pushState:true});console.log('Loaded AppRouter');$('#toggle-sidebar').click(function(){$('.main').toggleClass('main-open').toggleClass('main-close');$('.sidebar-container').toggleClass('sidebar-container-open').toggleClass('sidebar-container-close');setTimeout(function(){google.maps.event.trigger(this.map,'resize');}.bind(this),500);}.bind(this));this.isReady=true;google.maps.event.addListener(this.map,"rightclick",_.bind(this.contextMenuMap,this));google.maps.event.addListener(this.map,"idle",function(){if(!this.firstLoadDelay){this.fetchMarkers();}}.bind(this));google.maps.event.addListener(this.map,"click",_.bind(this.clickMap,this));this.sidebar.setResponsively();return this;},goToMyLocation:function(){if(typeof this.myLocation!=='undefined'){this.setCenterWithMarker(this.myLocation);}else if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){var latitude=position.coords.latitude;var longitude=position.coords.longitude;this.myLocation=new google.maps.LatLng(latitude,longitude);this.setCenterWithMarker(this.myLocation);this.createHighlightPoint(latitude,longitude,HIGHLIGHT_TYPE_USER_GPS);}.bind(this));}else{this.myLocation=this.defaultLocation;this.setCenterWithMarker(this.myLocation);}
this.map.setZoom(INIT_ZOOM);},closeInfoWindow:function(){if(app.infoWindow){this.selectedMarker=null;app.infoWindow.close();app.infoWindow=null;this.updateUrl();$(document).off('keydown',app.ESCinfoWindow);}},clickMap:function(e){this.closeInfoWindow();},trackDrag:function(){google.maps.event.addListener(this.map,"mousemove",function(){this.resetOnMouseUp=true;});},initLayers:function(severity){var severities=[SEVERITY_FATAL,SEVERITY_SEVERE,SEVERITY_LIGHT];var self=this;severities.forEach(function(severity){var attr=SEVERITY_ATTRIBUTES[severity];var layer=self.model.get(attr);if(!layer){self.model.set(attr,LAYERS[severity]);layer=LAYERS[severity];}});},loadMarker:function(model){for(var i=0;i<this.markerList.length;i++){if(this.markerList[i].model.get("type")==model.get("type")&&this.markerList[i].model.attributes.id==model.attributes.id){return;}}
if(this.clusterMode()||this.fitsFilters(model)||!this.clusterMode()&&model.get("type")==MARKER_TYPE_DISCUSSION){var markerView=new MarkerView({model:model,map:this.map,markerIconType:this.markerIconType}).render();model.set("markerView",this.markerList.length);this.markerList.push(markerView);}},loadCluster:function(model){var clusterExists=_.any(this.clusterList,function(cluster){return cluster.model.attributes.longitude===model.attributes.longitude&&cluster.model.attributes.latitude===model.attributes.latitude;});if(!clusterExists){var clusterView=new ClusterView({model:model,map:this.map}).render();this.clusterList.push(clusterView);}},loadClusters:function(model){this.oms.unspiderfy();this.clearClustersFromMap();this.clusters.each(_.bind(this.loadCluster,this));},fitsFilters:function(model){var layer=this.model.get(SEVERITY_ATTRIBUTES[model.get("severity")]);if(!layer){return false;}
return true;},loadMarkers:function(){this.oms.unspiderfy();this.clearMarkersFromMap();this.clearClustersFromMap();this.markers.each(_.bind(this.loadMarker,this));},clearMarkersFromMap:function(){if(this.markerList){_(this.markerList).each(_.bind(function(marker){marker.marker.setMap(null);},this));}
this.markerList=[];this.clusterer.clearMarkersFromMap();this.clearClustersFromMap();},clearClustersFromMap:function(){this.clusterer.removeClusters();this.clusterList=[];},resetMarkers:function(){this.clearMarkersFromMap();this.markers.reset();},resetClusters:function(){this.clearClustersFromMap();this.clusters.reset();},chooseMarker:function(){if(!this.markerList.length){return;}
var currentMarker=this.model.get("currentMarker");if(!this.markers.get(currentMarker)){this.updateUrl();return;}
var markerView=this.markerList[this.markers.get(currentMarker).get("markerView")];if(!markerView){return;}
markerView.choose();this.model.set("currentMarker",null);},contextMenuMap:function(e){this.clickLocation=e.latLng;if(this.menu){this.menu.remove();}
this.menu=new ContextMenuView({items:[{icon:"plus-sign",text:ADD_DISCUSSION,callback:_.bind(this.showDiscussion,this)},{icon:"plus-sign",text:NEW_FEATURES,callback:_.bind(this.featuresSubscriptionDialog,this)}]}).render(e);},addDiscussionMarker:function(){var identifier=this.newDiscussionIdentifier;if(typeof identifier=='undefined')return true;this.updateUrl(this.getDiscussionUrl(identifier));var model=new Discussion({identifier:identifier,latitude:this.clickLocation.lat(),longitude:this.clickLocation.lng(),type:MARKER_TYPE_DISCUSSION});var view=new MarkerView({model:model,map:this.map}).render();$.post("discussion",JSON.stringify({"latitude":model.get("latitude"),"longitude":model.get("longitude"),"identifier":identifier,"title":identifier}));return true;},showDiscussion:function(identifier){if(typeof identifier=='undefined'){identifier=this.clickLocation.toString();this.newDiscussionIdentifier=identifier;}else{this.updateUrl(this.getDiscussionUrl(identifier));this.newDiscussionIdentifier=undefined;}
$("#discussion-dialog").modal("show");this.discussionShown=identifier;$("#discussion-dialog").on("hidden",function(){this.discussionShown=null;}.bind(this));var url=window.location.protocol+"//"+window.location.host+"/discussion?identifier="+identifier;DISQUS.reset({reload:true,config:function(){this.page.identifier=identifier;this.page.url=url;this.page.title=identifier;}});},getDiscussionUrl:function(identifier){return"/?discussion="+identifier+"&"+app.getCurrentUrlParams();},featuresSubscriptionDialog:function(type,event){if(this.createDialog)this.createDialog.close();this.createDialog=new FeatureDialog({type:type,event:event,markers:this.markers}).render();},handleSearchBox:function(){var places=this.searchBox.getPlaces();if(places&&places.length>0){var place=places[0];this.createHighlightPoint(place.geometry.location.lat(),place.geometry.location.lng(),HIGHLIGHT_TYPE_USER_SEARCH);this.setCenterWithMarker(place.geometry.location);this.map.setZoom(INIT_ZOOM);}},createHighlightPoint:function(lat,lng,highlightPointType){if(isNaN(lat)||isNaN(lng)||isNaN(highlightPointType))return;$.post("highlightpoints",JSON.stringify({"latitude":lat,"longitude":lng,"type":highlightPointType}));},setCenterWithMarker:function(loc){this.closeInfoWindow();this.map.setCenter(loc);this.fetchMarkers();if(this.locationMarker){this.locationMarker.setMap(null);}
this.locationMarker=new google.maps.Marker({position:loc,map:this.map,icon:{path:google.maps.SymbolPath.CIRCLE,scale:0,fillColor:'black'},title:'מיקום נוכחי'});if(tourLocation==2)
{tourLocation=3;var location=this.locationMarker;contentString='<p>המיקום שחיפשתם יסומן באיקון הזה. </br> מסביבו תוכלו לראות אייקונים שמייצגים תאונות עם נפגעים.  </p>';titleString=' המיקום שחיפשתם ';defInfoWindows();infowindow=new google.maps.InfoWindow({content:htmlTourString,maxWidth:350});infowindow.open(this.map,location);tourStyle(infowindow);}},getCurrentUrlParams:function(){var params=this.buildMarkersParams(true);var returnParams=[];$.each(params,function(attr,attr_value){returnParams.push(attr+"="+attr_value);});return returnParams.join("&");},ESCinfoWindow:function(event){if(event.keyCode==27){app.closeInfoWindow();console.log('ESC pressed');}},retinaIconsResize:function(image_url){if(isRetina){image_url={url:image_url,scaledSize:new google.maps.Size(30,50)};}
return image_url;},toggleMarkerIconType:function(){this.markerIconType=!this.markerIconType;this.resetMarkers();this.fetchMarkers();},loadFilter:function(){if($("#checkbox-discussions").is(":checked")){this.show_discussions='1'}else{this.show_discussions=''}
...
if($("#checkbox-urban").is(":checked")&&$("#checkbox-nonurban").is(":checked")){this.show_urban=3;}else if($("#checkbox-urban").is(":checked")){this.show_urban=2;}else if($("#checkbox-nonurban").is(":checked")){this.show_urban=1;}else{this.show_urban=0;};if($("#checkbox-intersection").is(":checked")&&$("#checkbox-nonintersection").is(":checked")){this.show_intersection=3;}else if($("#checkbox-intersection").is(":checked")){this.show_intersection=2;}else if($("#checkbox-nonintersection").is(":checked")){this.show_intersection=1;}else{this.show_intersection=0;};if($("#checkbox-multi-lane").is(":checked")&&$("#checkbox-one-lane").is(":checked")){this.show_lane=3;}else if($("#checkbox-multi-lane").is(":checked")){this.show_lane=2;}else if($("#checkbox-one-lane").is(":checked")){this.show_lane=1;}else{this.show_lane=0;};this.weather=$("input[type='radio'][name='weather']:checked").val();this.road=$("input[type='radio'][name='road']:checked").val();this.separation=$("input[type='radio'][name='separation']:checked").val();this.surface=$("input[type='radio'][name='surface']:checked").val();this.acctype=$("input[type='radio'][name='acctype']:checked").val();this.controlmeasure=$("input[type='radio'][name='controlmeasure']:checked").val();this.district=$("input[type='radio'][name='district']:checked").val();this.case_type=$("input[type='radio'][name='casetype']:checked").val();this.dateRanges=[new Date($("#sdate").val()),new Date($("#edate").val())]
this.resetMarkers();this.fetchMarkers();this.updateFilterString();},loadFilterFromParameters:function(){var bool_atrs={};bool_atrs["checkbox-discussions"]=this.show_discussions;bool_atrs["checkbox-accidents"]=this.show_markers;bool_atrs["checkbox-accurate"]=this.accurate;bool_atrs["checkbox-approx"]=this.approx;bool_atrs["checkbox-fatal"]=this.show_fatal;bool_atrs["checkbox-severe"]=this.show_severe;bool_atrs["checkbox-light"]=this.show_light;$.each(bool_atrs,function(attr,attr_value){$('#'+attr).prop("checked",attr_value=='1');});if(this.show_urban==3){$("#checkbox-urban").prop("checked",true);$("#checkbox-nonurban").prop("checked",true);}else if(this.show_urban==2){$("#checkbox-urban").prop("checked",true);$("#checkbox-nonurban").prop("checked",false);}else if(this.show_urban==1){$("#checkbox-nonurban").prop("checked",true);$("#checkbox-urban").prop("checked",false);}else{$("#checkbox-nonurban").prop("checked",false);$("#checkbox-urban").prop("checked",false);}
if(this.show_intersection==3){$("#checkbox-intersection").prop("checked",true);$("#checkbox-nonintersection").prop("checked",true);}else if(this.show_intersection==2){$("#checkbox-intersection").prop("checked",true);$("#checkbox-nonintersection").prop("checked",false);}else if(this.show_intersection==1){$("#checkbox-nonintersection").prop("checked",true);$("#checkbox-intersection").prop("checked",false);}else{$("#checkbox-nonintersection").prop("checked",false);$("#checkbox-intersection").prop("checked",false);}
if(this.show_lane==3){$("#checkbox-multi-lane").prop("checked",true);$("#checkbox-one-lane").prop("checked",true);}else if(this.show_lane==2){$("#checkbox-multi-lane").prop("checked",true);$("#checkbox-one-lane").prop("checked",false);}else if(this.show_lane==1){$("#checkbox-one-lane").prop("checked",true);$("#checkbox-multi-lane").prop("checked",false);}else{$("#checkbox-one-lane").prop("checked",false);$("#checkbox-multi-lane").prop("checked",false);}
var radio_attrs={};radio_attrs["weather"]=this.weather;radio_attrs["road"]=this.road;radio_attrs["separation"]=this.separation;radio_attrs["surface"]=this.surface;radio_attrs["acctype"]=this.acctype;radio_attrs["controlmeasure"]=this.controlmeasure;radio_attrs["district"]=this.district;radio_attrs["case_type"]=this.case_type;$.each(radio_attrs,function(attr,attr_value){$("input[type='radio'][name='"+attr+"']").each(function(){if($(this).val()==attr_value){$(this).prop("checked",true);}});});if(this.dateRanges!=='undefined'){document.getElementById("sdate").valueAsDate=new Date(this.dateRanges[0]);document.getElementById("edate").valueAsDate=new Date(this.dateRanges[1]);}},changeDate:function(){var start_date,end_date;if($("#checkbox-2014").is(":checked")){start_date="2014";end_date="2015"}
...
$("#sdate").val(start_date+'-01-01');$("#edate").val(end_date+'-01-01');this.show_day=$("input[type='radio'][name='day']:checked").val()
this.show_holiday=$("input[type='radio'][name='holiday']:checked").val()
this.show_time=$("input[type='radio'][name='time']:checked").val()
if(!isNaN(parseInt($("#stime").val()))&&!isNaN(parseInt($("#etime").val()))){this.start_time=parseInt($("#stime").val())
this.end_time=parseInt($("#etime").val())
$("#checkbox-time-all").prop('checked',true);}
this.dateRanges=[new Date(start_date+'-01-01'),new Date(end_date+'-01-01')];this.resetMarkers();this.fetchMarkers();this.updateFilterString();},updateFilterString:function(){if(!this.clusterMode()){var fatal=this.show_fatal,severe=this.show_severe,light=this.show_light,severityText=" בחומרה ";var accurate=this.accurate,approx=this.approx,accuracyText=" ובעיגון ";if(fatal=='1'){fatal="קטלנית "}else{fatal=""}
if(severe=='1'&&fatal!=''&&light==''){severe="וקשה ";}else if(severe=='1'){severe="קשה ";}else{severe='';};if(fatal==''&&severe==''&&light==''){severityText=""}
if(light=='1'&&(fatal!=''||severe!='')){light="וקלה ";}else if(light=='1'){light="קלה ";}else{light='';}
if(accurate=='1'){accurate="מדויק "}else{accurate=""}
if(approx=='1'&&accurate!=''){approx="ומרחבי";}else if(approx=='1'){approx="מרחבי ";}else{approx="";}
if(accurate==''&&approx==''){accuracyText=""}
$("#filter-string").empty().append("<span>מציג </span>").append("<span><a onclick='showFilter(FILTER_MARKERS)'>"+markerCount+"</a></span>").append("<span> תאונות</span>").append("<span> בין התאריכים </span><br>").append("<span><a onclick='showFilter(FILTER_DATE)'>"+
moment(this.dateRanges[0]).format('LL')+" עד "+
moment(this.dateRanges[1]).format('LL')+"</a></span><br>").append("<span>"+severityText+"</span>").append("<span><a onclick='showFilter(FILTER_INFO)' style='color: #d81c32;'>"+fatal+"</a></span>").append("<span><a onclick='showFilter(FILTER_INFO)' style='color: #ff9f1c;'>"+severe+"</a></span>").append("<span><a onclick='showFilter(FILTER_INFO)' style='color: #ffd82b;'>"+light+"</a></span><br>").append("<span>"+accuracyText+"</span>").append("<span><a onclick='showFilter(FILTER_INFO)'>"+accurate+"</a></span>").append("<span><a onclick='showFilter(FILTER_INFO)'>"+approx+"</a></span>");}else{$("#filter-string").empty().append("<p>התקרב על מנת לקבל נתוני סינון</p>");}}});});var localization={"SUG_DEREH":{"1":"עירוני בצומת","2":"עירוני לא בצומת","3":"לא עירוני בצומת","4":"לא עירוני לא בצומת"},"YEHIDA":{"11":"מרחב חוף (חיפה)","12":"מרחב גליל","14":"מרחב עמקים","20":"מרחב ת\"א","33":"מרחב אילת","34":"מרחב הנגב","36":"מרחב שמשון (עד 1999)","37":"מרחב שמשון (החל ב-2004)","38":"מרחב לכיש","41":"מרחב שומרון","43":"מרחב יהודה","51":"מרחב השרון","52":"מרחב השפלה","61":"מחוז ירושלים"},"SUG_YOM":{"1":"חג","2":"ערב חג","3":"חול המועד","4":"יום אחר",},"HUMRAT_TEUNA":{"1":"קטלנית","2":"קשה","3":"קלה",},"SUG_TEUNA":{"1":"פגיעה בהולך רגל","2":"התנגשות חזית אל צד","3":"התנגשות חזית באחור","4":"התנגשות צד בצד","5":"התנגשות חזית אל חזית","6":"התנגשות עם רכב שנעצר ללא חניה","7":"התנגשות עם רכב חונה","8":"התנגשות עם עצם דומם","9":"ירידה מהכביש או עלייה למדרכה","10":"התהפכות","11":"החלקה","12":"פגיעה בנוסע בתוך כלי רכב","13":"נפילה ברכב נע","14":"שריפה","15":"אחר","17":"התנגשות אחור אל חזית","18":"התנגשות אחור אל צד","19":"התנגשות עם בעל חיים","20":"פגיעה ממטען של רכב","21":"איחוד הצלה",},"ZURAT_DEREH":{"1":"כניסה למחלף","2":"ביציאה ממחלף","3":"מ.חניה/ת. דלק","4":"שיפוע תלול","5":"עקום חד","6":"על גשר מנהרה","7":"מפגש מסילת ברזל","8":"כביש ישר/צומת","9":"אחר",},"HAD_MASLUL":{"1":"חד סיטרי","2":"דו סיטרי+קו הפרדה רצוף","3":"דו סיטרי אין קו הפרדה רצוף","4":"אחר",},"RAV_MASLUL":{"1":"מיפרדה מסומנת בצבע","2":"מיפרדה עם גדר בטיחות","3":"מיפרדה בנויה ללא גדר בטיחות","4":"מיפרדה לא בנויה","5":"אחר",},"MEHIRUT_MUTERET":{"1":"עד 50 קמ\"ש","2":"60 קמ\"ש","3":"70 קמ\"ש","4":"80 קמ\"ש","5":"90 קמ\"ש","6":"100 קמ\"ש",},"TKINUT":{"1":"אין ליקוי","2":"שוליים גרועים","3":"כביש משובש","4":"שוליים גרועים וכביש משובש",},"ROHAV":{"1":"עד 5 מטר","2":"5 עד 7","3":"7 עד 10.5","4":"10.5 עד 14","5":"יותר מ-14",},"SIMUN_TIMRUR":{"1":"סימון לקוי/חסר","2":"תימרור לקוי/חסר","3":"אין ליקוי","4":"לא נדרש תמרור",},"TEURA":{"1":"אור יום רגיל","2":"ראות מוגבלת עקב מזג אויר (עשן,ערפל)","3":"לילה פעלה תאורה","4":"קיימת תאורה בלתי תקינה/לא פועלת","5":"לילה לא קיימת תאורה",},"BAKARA":{"1":"אין בקרה","2":"רמזור תקין","3":"רמזור מהבהב צהוב","4":"רמזור לא תקין","5":"תמרור עצור","6":"תמרור זכות קדימה","7":"אחר",},"MEZEG_AVIR":{"1":"בהיר","2":"גשום","3":"שרבי","4":"ערפילי","5":"אחר",},"PNE_KVISH":{"1":"יבש","2":"רטוב ממים","3":"מרוח בחומר דלק","4":"מכוסה בבוץ","5":"חול או חצץ על הכביש","6":"אחר",},"SUG_EZEM":{"1":"עץ","2":"עמוד חשמל/תאורה/טלפון","3":"תמרור ושלט","4":"גשר סימניו ומגיניו","5":"מבנה","6":"גדר בטיחות לרכב","7":"חבית","8":"אחר",},"MERHAK_EZEM":{"1":"עד מטר","2":"1-3 מטר","3":"על הכביש","4":"על שטח הפרדה",},"LO_HAZA":{"1":"הלך בכיוון התנועה","2":"הלך נגד","3":"שיחק על הכביש","4":"עמד על הכביש","5":"היה על אי הפרדה","6":"היה על שוליים/מדרכה","7":"אחר",},"OFEN_HAZIYA":{"1":"התפרץ אל הכביש","2":"חצה שהוא מוסתר","3":"חצה רגיל","4":"אחר",},"MEKOM_HAZIYA":{"1":"לא במעבר חציה ליד צומת","2":"לא במעבר חציה לא ליד צומת","3":"במעבר חציה בלי רמזור","4":"במעבר חציה עם רמזור",},"KIVUN_HAZIYA":{"1":"מימין לשמאל","2":"משמאל לימין",},"STATUS_IGUN":{"1":"עיגון מדויק","2":"מרכז ישוב","3":"מרכז דרך","4":"מרכז קילומטר","9":"לא עוגן",}}
var fields={"pk_teuna_fikt":"מזהה","SUG_DEREH":"סוג דרך","SHEM_ZOMET":"שם צומת","SEMEL_YISHUV":"ישוב","REHOV1":"רחוב 1","REHOV2":"רחוב 2","BAYIT":"מספר בית","ZOMET_IRONI":"צומת עירוני","KVISH1":"כביש 1","KVISH2":"כביש 2","ZOMET_LO_IRONI":"צומת לא עירוני","YEHIDA":"יחידה","SUG_YOM":"סוג יום","RAMZOR":"רמזור","HUMRAT_TEUNA":"חומרת תאונה","SUG_TEUNA":"סוג תאונה","ZURAT_DEREH":"צורת דרך","HAD_MASLUL":"חד מסלול","RAV_MASLUL":"רב מסלול","MEHIRUT_MUTERET":"מהירות מותרת","TKINUT":"תקינות","ROHAV":"רוחב","SIMUN_TIMRUR":"סימון תמרור","TEURA":"תאורה","BAKARA":"בקרה","MEZEG_AVIR":"מזג אוויר","PNE_KVISH":"פני כביש","SUG_EZEM":"סוג עצם","MERHAK_EZEM":"מרחק עצם","LO_HAZA":"לא חצה","OFEN_HAZIYA":"אופן חציה","MEKOM_HAZIYA":"מקום חציה","KIVUN_HAZIYA":"כיוון חציה","STATUS_IGUN":"עיגון","MAHOZ":"מחוז","NAFA":"נפה","EZOR_TIVI":"אזור טבעי","MAAMAD_MINIZIPALI":"מעמד מוניציפלי","ZURAT_ISHUV":"צורת יישוב","SUG_MEORAV":"סוג מעורב","SHNAT_HOZAA":"שנת הוצאת רשיון נהיגה","KVUZA_GIL":"קבוצת גיל","MIN":"מין","SUG_REHEV_NASA_LMS":"סוג רכב בו נסע","EMZAE_BETIHUT":"אמצעי בטיחות","HUMRAT_PGIA":"חומרת פגיעה","SUG_NIFGA_LMS":"סוג נפגע","PEULAT_NIFGA_LMS":"מיקום פצוע","KVUTZAT_OHLUSIYA_LMS":"קבוצת אוכלוסיה","MAHOZ_MEGURIM":"מחוז מגורים","NAFA_MEGURIM":"נפת מגורים","EZOR_TIVI_MEGURIM":"אזור טבעי מגורים","MAAMAD_MINIZIPALI_MEGURIM":"מעמד מוניצפלי מגורים","ZURAT_ISHUV_MEGURIM":"צורת ישוב מגורים","PAZUA_USHPAZ":"משך אשפוז","MADAD_RAFUI":"מדד רפואי לחומרת הפציעה - ISS","YAAD_SHIHRUR":"יעד שחרור","SHIMUSH_BE_AVIZAREY_BETIHOT":"שימוש באביזרי בטיחות","PTIRA_MEUHERET":"מועד הפטירה","NEFAH":"נפח מנוע","SHNAT_YITZUR":"שנת ייצור","KIVUNE_NESIA":"כיוון נסיעה","MATZAV_REHEV":"מצב רכב","SHIYUH_REHEV_LMS":"שיוך רכב","SUG_REHEV_LMS":"סוג רכב","MEKOMOT_YESHIVA_LMS":"מקומות ישיבה","MISHKAL_KOLEL_LMS":"משקל כולל","ACC_ID":"מספר סידורי","PROVIDER_CODE":"סוג תיק"}
var ACCYEARS={"\u05e9\u05e0\u05ea 2015":["01/01/2015","31/12/2015"],"\u05e9\u05e0\u05ea 2014":["01/01/2014","31/12/2014"]};var inv_dict={"SUG_MEORAV":{"1":"נהג שלא נפגע","2":"נהג נפגע","3":"נפגע שאינו נהג",},"MIN":{"1":"זכר","2":"נקבה",},"SUG_REHEV_NASA_LMS":{"1":"רכב נוסעים פרטי","2":"משא עד 3.5 טון אחוד","3":"משא עד 3.5 טון לא אחוד","4":"משא 1.4 עד 9.9 טון משקל כולל","5":"משא 10 עד 15.9 טון משקל כולל","6":"משא 16 עד 33.9 טון משקל כולל","7":"משא 34.0+טון משקל כולל","8":"אופנוע עד 50 סמ״ק","9":"אופנוע 51 עד 250 סמ״ק","10":"אופנוע 251 עד 500 סמ״ק","11":"אוטובוס","12":"מונית","13":"רכב עבודה","14":"טרקטור","15":"אופניים","16":"רכבת","17":"אחר","18":"אוטובוס זעיר","19":"אופנוע 501+ סמ״ק",},"EMZAE_BETIHUT":{"1":"חגר חגורה","2":"חבש קסדת מגן (רק לאופנוע)","3":"ישב במושב/ריתמה לילדים (רק לנפגע ילד)","4":"לא השתמש באמצעי","5":"לא ידוע",},"HUMRAT_PGIA":{"1":"הרוג","2":"פצוע קשה","3":"פצוע קל",},"SUG_NIFGA_LMS":{"1":"הולך רגל","2":"נהג-רכב בעל 4+ גלגלים","3":"נוסע- רכב בעל 4+ גלגלים","4":"נהג-אופנוע","5":"נוסע-אופנוע (לא נהג)","6":"נהג-אופניים","7":"נוסע-אופניים (לא נהג)","8":"נהג-רכב לא ידוע","9":"נוסע-רכב לא ידוע",},"PEULAT_NIFGA_LMS":{"1":"ישב ברכב במושב קדמי","2":"ישב ברכב במושב אחורי","3":"ישב ברכב בארגז מאחור","4":"עמד ברכב","5":"עלה לרכב","6":"ירד מהרכב","7":"פעולה אחרת","8":"לא ידוע","9":"הולך רגל",},"PAZUA_USHPAZ":{"1":"אשפוז עד 24 שעות","2":"אשפוז מעל 24 שעות",},"MADAD_RAFUI":{"1":"קל (1-8)","2":"בינוני (9-15)","3":"קשה (16-24)","4":"קשה מאוד (25-75)",},"YAAD_SHIHRUR":{"1":"בית","2":"בית חולים אחר","3":"מוסד שיקומי/ מחלקה שיקומית","4":"מוות","5":"אחר",},"SHIMUSH_BE_AVIZAREY_BETIHOT":{"1":"כן","2":"לא","3":"לא ידוע",},"PTIRA_MEUHERET":{"1":"פטירה רגילה עד 30 יום","2":"פטירה מאוחרת מעל 30 יום",}}
var veh_dict={"MATZAV_REHEV":{"1":"גנוב","2":"השכרה","3":"לימוד","4":"הסעת ילדים",},"SHIYUH_REHEV_LMS":{"0":"לא מנועי","1":"ישראלי","2":"צבאי","3":"משטרתי","4":"זר","5":"דיפלומטי","6":"יש״ע","9":"אלמוני",},"SUG_REHEV_LMS":{"1":"רכב נוסעים פרטי","2":"משא עד 4 טון אחוד","3":"משא עד 4 טון לא אחוד","4":"משא 1.4 עד 9.9 טון משקל כולל","5":"משא 10 עד 15.9 טון משקל כולל","6":"משא 16 עד 33.9 טון משקל כולל","7":"משא 34.0+טון משקל כולל","8":"אופנוע עד 50 סמ״ק","9":"אופנוע 51 עד 250 סמ״ק","10":"אופנוע 251 עד 500 סמ״ק","11":"אוטובוס","12":"מונית","13":"רכב עבודה","14":"טרקטור","15":"אופניים","16":"רכבת","17":"אחר","18":"אוטובוס זעיר","19":"אופנוע מעל 500 סמ״ק",}}
var isRetina=(window.devicePixelRatio>1||(window.matchMedia&&window.matchMedia("(-webkit-min-device-pixel-ratio: 1.5),(-moz-min-device-pixel-ratio: 1.5),(min-device-pixel-ratio: 1.5)").matches));if(isRetina){function addRetinaSuffix(url){return url.replace(/(^.*)(\.png)$/gi,function switchFunction(x,y1,y2){return y1+"@2x"+y2;});}
for(var icon in ICONS){for(var i in ICONS[icon]){ICONS[icon][i]=addRetinaSuffix(ICONS[icon][i]);}}
for(var icon in MULTIPLE_ICONS){MULTIPLE_ICONS[icon]=addRetinaSuffix(MULTIPLE_ICONS[icon]);}
USER_LOCATION_ICON=addRetinaSuffix(USER_LOCATION_ICON);DISCUSSION_ICON=addRetinaSuffix(DISCUSSION_ICON);}
﻿var jsPanelInst;var currentTypeOfChart="accidentsPerMonth";var currentDataType="numberType";function statPanelClick(widthOfPanel,heightOfPanel,chartWidth,chartHeight){jQuery.jsPanel({rtl:{rtl:true,lang:'he'},size:{width:widthOfPanel,height:heightOfPanel},title:'קצת גרפים',position:'center',id:'statPanel',theme:'light',callback:function(jsPanel){jsPanelInst=jsPanel;startJSPanelWithChart(jsPanel,widthOfPanel,heightOfPanel,chartWidth,chartHeight);}});};var startJSPanelWithChart=function(jsPanel,widthOfPanel,heightOfPanel,chartWidth,chartHeight){var isClusterMode=false;var str='<div>';str+='<select class="form-control" id="selectTypeOfChart">';str+='<option value="accidentsPerMonth" id="accidentsPerMonth">מספר התאונות בחודש על פני כל השנים שנבחרו</option>';str+='<option value="accidentsPerMonthStacked" id="accidentsPerMonthStacked">מספר התאונות בחודש - מקובץ לפי חודשים</option>';str+='<option value="severityTotal" id="severityTotal">מספר התאונות לפי דרגת חומרה</option>';str+='<option value="severityPerMonth" id="severityPerMonth">מספר התאונות בחודש לפי דרגת חומרה</option>';str+='<option value="severityPerMonthStacked" id="severityPerMonthStacked">מספר התאונות בחודש לפי דרגת חומרה - מקובץ לפי חודשים</option>';str+='</select>';str+='<select class="form-control" id="selectTypeOfData">';str+='<option value="numberType" id="numberType">מספרים</option>';str+='<option value="percentType" id="percentType">אחוזים</option>';str+='</select>';str+='</div>';str+='<div id="myChart" width="';str+=chartWidth.toString();str+='" height="';str+=chartHeight.toString()+'">';if(app.markers.length===0&&app.clusters.length>0){isClusterMode=true;str+='<h2> אין נתונים להצגה. אנא הגדל/י את רמת המיקוד במפה </h2>'}
str+='</div>';jsPanel.content.empty();jsPanel.content.append(str);$("#selectTypeOfChart").width(360);$("#selectTypeOfData").width(100);$("#"+currentTypeOfChart).prop('selected',true);$("#"+currentDataType).prop('selected',true);if(isClusterMode){return;}
switch(currentTypeOfChart){case"accidentsPerMonth":var sum;var jsonAccidentsByMonth;var groupedAccidentsByMonth=_.countBy(app.markers.pluck("created"),function(item){return item.substring(0,7);});if(currentDataType==="percentType"){sum=_.reduce(groupedAccidentsByMonth,function(num1,num2){return num1+num2;},0);jsonAccidentsByMonth=_.map(groupedAccidentsByMonth,function(numOfAccidents,month){return{"label":month,"value":(numOfAccidents/sum).toFixed(2)}});}else{jsonAccidentsByMonth=_.map(groupedAccidentsByMonth,function(numOfAccidents,month){return{"label":month,"value":numOfAccidents.toString()}});}
jsonAccidentsByMonth=_.sortBy(jsonAccidentsByMonth,'label');if(currentDataType==="percentType"){FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"column2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש באחוזים","xAxisName":"חודשים","yAxisName":"אחוז התאונות מסך התאונות","theme":"fint","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25","numberScaleValue":".01","numberScaleUnit":"%",},"data":jsonAccidentsByMonth}});statChart.render();});}else{FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"column2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש","xAxisName":"חודשים","yAxisName":"מספר תאונות","theme":"fint","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25",},"data":jsonAccidentsByMonth}});statChart.render();});}
break;case"accidentsPerMonthStacked":var groupedAccidentsByYear=_.countBy(app.markers.pluck("created"),function(item){return item.substring(0,4);});var jsonAccidentsByYear=_.map(groupedAccidentsByYear,function(numOfAccidents,year){return{"label":year,"value":numOfAccidents.toString()}});jsonAccidentsByYear=_.sortBy(jsonAccidentsByYear,'label');var groupedAccidentsByMonth=_.countBy(app.markers.pluck("created"),function(item){return item.substring(5,7);});var numberOfYears=jsonAccidentsByYear.length;var jsonAccidentsByMonth=_.map(groupedAccidentsByMonth,function(numOfAccidents,month){return{"label":month}});jsonAccidentsByMonth=_.sortBy(jsonAccidentsByMonth,'label');var groupedAccidentsForAllYears=_.countBy(app.markers.pluck("created"),function(item){return item.substring(0,7);});var jsonAccidentsByMonthAllYears=_.map(groupedAccidentsForAllYears,function(numOfAccidents,month){return{"label":month,"value":numOfAccidents.toString()}});jsonAccidentsByMonthAllYears=_.sortBy(jsonAccidentsByMonthAllYears,'label');var dataPerMonth=[];var dataObj={};var dataOfValues=[];var dataValue={};var counter;for(i=0;i<numberOfYears;i++){var numOfMonthTotal=jsonAccidentsByMonthAllYears.length;dataOfValues=[];counter=0;for(j=0;j<numOfMonthTotal;j++){dataValue=new Object();if(jsonAccidentsByMonthAllYears[j]["label"].substring(0,4)===jsonAccidentsByYear[i]["label"]){if(currentDataType==="percentType"){dataValue={value:(jsonAccidentsByMonthAllYears[j]["value"]/groupedAccidentsByMonth[jsonAccidentsByMonthAllYears[j]["label"].substring(5,7)]).toFixed(2)};}else{dataValue={value:jsonAccidentsByMonthAllYears[j]["value"].toString()};}
jsonAccidentsByMonthAllYears.splice(j,1);j--;numOfMonthTotal--;}else{dataValue={value:"0"};}
dataOfValues.push(dataValue);}
dataObj=new Object();dataObj={seriesname:jsonAccidentsByYear[i]["label"].toString(),data:dataOfValues};dataPerMonth.push(dataObj);counter++;numOfMonthTotal=jsonAccidentsByMonthAllYears.length;if(counter>11){break;}}
var categories={category:jsonAccidentsByMonth};if(currentDataType==="percentType"){FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"stackedcolumn2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש באחוזים","xAxisName":"חודשים - מקובץ מכל השנים שנבחרו","yAxisName":"אחוז התאונות מסך התאונות החודשי","theme":"fint","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25","stack100Percent":"1","decimals":"0",},"categories":categories,"dataset":dataPerMonth}});statChart.render();});}else{FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"stackedcolumn2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש","xAxisName":"חודשים - מקובץ מכל השנים שנבחרו","yAxisName":"מספר תאונות","theme":"fint","showSum":"1","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25",},"categories":categories,"dataset":dataPerMonth}});statChart.render();});}
break;case"severityTotal":var sum;var jsonAccidentsBySeverity;var groupedAccidentsBySeverity=_.countBy(app.markers.pluck("severity"),function(item){return item;});if(currentDataType==="percentType"){sum=_.reduce(groupedAccidentsBySeverity,function(num1,num2){return num1+num2;},0);jsonAccidentsBySeverity=_.map(groupedAccidentsBySeverity,function(numOfAccidents,severity){return{"label":severity,"value":(numOfAccidents/sum).toFixed(2)}});}else{jsonAccidentsBySeverity=_.map(groupedAccidentsBySeverity,function(numOfAccidents,severity){return{"label":severity,"value":numOfAccidents.toString()}});}
jsonAccidentsBySeverity=_.sortBy(jsonAccidentsBySeverity,'label');for(i=0;i<jsonAccidentsBySeverity.length;i++){switch(jsonAccidentsBySeverity[i]["label"]){case"1":jsonAccidentsBySeverity[i]["label"]="קטלנית";jsonAccidentsBySeverity[i]["color"]="#d81c32";break;case"2":jsonAccidentsBySeverity[i]["label"]="קשה";jsonAccidentsBySeverity[i]["color"]="#ff9f1c";break;case"3":jsonAccidentsBySeverity[i]["label"]="קלה";jsonAccidentsBySeverity[i]["color"]="#ffd82b";break;}}
if(currentDataType==="percentType"){FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"column2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות לפי חומרה באחוזים","xAxisName":"חומרת התאונות","yAxisName":"אחוז התאונות מסך התאונות","theme":"fint","showSum":"1","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25","numberScaleValue":".01","numberScaleUnit":"%",},"data":jsonAccidentsBySeverity}});statChart.render();});}else{FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"column2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות לפי חומרה","xAxisName":"חומרת התאונות","yAxisName":"מספר תאונות","theme":"fint","showSum":"1","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25",},"data":jsonAccidentsBySeverity}});statChart.render();});}
break;case"severityPerMonth":case"severityPerMonthStacked":var models=[];var doesAlreadyExists;var monthCategories=[];var dataset=[];var dataValuesLethal=[];var dataValuesSevere=[];var dataValuesLight=[];var lastDate;var currentDate;var found;app.markers.each(function(model){if(currentTypeOfChart==="severityPerMonth"){models.push({created:model.get("created").substring(0,7),severity:model.get("severity")});}else{models.push({created:model.get("created").substring(5,7),severity:model.get("severity")});}});models=_.sortBy(models,'created');if(models.length>0){lastDate=models[0]["created"];}
for(var index=0;index<models.length;index++){doesAlreadyExists=false;currentDate=models[index]["created"];if(currentDate!=lastDate){PushZeroValues(dataValuesLethal,dataValuesSevere,dataValuesLight,lastDate);lastDate=currentDate;}
switch(models[index]["severity"]){case 1:for(i=0;i<dataValuesLethal.length;i++){if(dataValuesLethal[i]["label"]===models[index]["created"]){dataValuesLethal[i]["value"]++;doesAlreadyExists=true;break;}}
if(doesAlreadyExists===false){dataValuesLethal.push({label:models[index]["created"],value:1});}
break;case 2:for(i=0;i<dataValuesSevere.length;i++){if(dataValuesSevere[i]["label"]===models[index]["created"]){dataValuesSevere[i]["value"]++;doesAlreadyExists=true;break;}}
if(doesAlreadyExists===false){dataValuesSevere.push({label:models[index]["created"],value:1});}
break;case 3:for(i=0;i<dataValuesLight.length;i++){if(dataValuesLight[i]["label"]===models[index]["created"]){dataValuesLight[i]["value"]++;doesAlreadyExists=true;break;}}
if(doesAlreadyExists===false){dataValuesLight.push({label:models[index]["created"],value:1});}
break;}
found=monthCategories.some(function(el){return el.label===models[index]["created"];});if(!found){monthCategories.push({label:models[index]["created"]});}}
PushZeroValues(dataValuesLethal,dataValuesSevere,dataValuesLight,lastDate);dataValuesLethal=_.sortBy(dataValuesLethal,'label');dataValuesSevere=_.sortBy(dataValuesSevere,'label');dataValuesLight=_.sortBy(dataValuesLight,'label');if(currentDataType==="percentType"){for(var index=0;index<dataValuesLethal.length;index++){dataValuesLethal[index]["value"]=(parseFloat(dataValuesLethal[index]["value"])/(parseFloat(dataValuesLethal[index]["value"])+
parseFloat(dataValuesSevere[index]["value"])+
parseFloat(dataValuesLight[index]["value"]))).toFixed(2);delete dataValuesLethal[index]["label"];}
for(var index=0;index<dataValuesSevere.length;index++){dataValuesSevere[index]["value"]=(parseFloat(dataValuesSevere[index]["value"])/(parseFloat(dataValuesLethal[index]["value"])+
parseFloat(dataValuesSevere[index]["value"])+
parseFloat(dataValuesLight[index]["value"]))).toFixed(2);delete dataValuesSevere[index]["label"];}
for(var index=0;index<dataValuesLight.length;index++){dataValuesLight[index]["value"]=(parseFloat(dataValuesLight[index]["value"])/(parseFloat(dataValuesLethal[index]["value"])+
parseFloat(dataValuesSevere[index]["value"])+
parseFloat(dataValuesLight[index]["value"]))).toFixed(2);delete dataValuesLight[index]["label"];}}else{for(var index=0;index<dataValuesLethal.length;index++){delete dataValuesLethal[index]["label"];}
for(var index=0;index<dataValuesSevere.length;index++){delete dataValuesSevere[index]["label"];}
for(var index=0;index<dataValuesLight.length;index++){delete dataValuesLight[index]["label"];}}
dataset.push({seriesname:"קטלנית",data:dataValuesLethal,color:"#d81c32"});dataset.push({seriesname:"קשה",data:dataValuesSevere,color:"#ff9f1c"});dataset.push({seriesname:"קלה",data:dataValuesLight,color:"#ffd82b"});monthCategories=_.sortBy(monthCategories,'label');for(var index=0;index<monthCategories.length;index++){switch(monthCategories[index]["label"]){case"1":monthCategories[index]["label"]="קטלנית";break;case"2":monthCategories[index]["label"]="קשה";break;case"3":monthCategories[index]["label"]="קלה";break;}}
var categories={category:monthCategories};if(currentDataType==="percentType"){FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"stackedcolumn2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש לפי חומרה באחוזים","xAxisName":"חודשים - מקובץ מכל השנים שנבחרו","yAxisName":"מספר תאונות לפי חומרה באחוזים","theme":"fint","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25","stack100Percent":"1","decimals":"0",},"categories":categories,"dataset":dataset}});statChart.render();});}else{FusionCharts.ready(function(){var statChart=new FusionCharts({"type":"stackedcolumn2d","renderAt":"myChart","width":chartWidth,"height":chartHeight,"dataFormat":"json","dataSource":{"chart":{"caption":"מספר התאונות בחודש לפי חומרה","xAxisName":"חודשים - מקובץ מכל השנים שנבחרו","yAxisName":"מספר תאונות לפי חומרה","theme":"fint","showSum":"1","labelFontSize":"15","yAxisNameFontSize":"20","xAxisNameFontSize":"20","captionFontSize":"25",},"categories":categories,"dataset":dataset}});statChart.render();});}
break;}
$("#selectTypeOfChart").on("change",function(){if(jsPanelInst!=null){currentTypeOfChart=$("#selectTypeOfChart").val();startJSPanelWithChart(jsPanelInst,$("#statPanel").width(),$("#statPanel").height(),$("#statPanel").width()-30,$("#statPanel").height()-80);}});$("#selectTypeOfData").on("change",function(){if(jsPanelInst!=null){currentDataType=$("#selectTypeOfData").val();startJSPanelWithChart(jsPanelInst,$("#statPanel").width(),$("#statPanel").height(),$("#statPanel").width()-30,$("#statPanel").height()-80);}});};function PushZeroValues(dataValuesLethal,dataValuesSevere,dataValuesLight,lastDate){if(dataValuesLethal.length>0){if(dataValuesLethal[dataValuesLethal.length-1]["label"]!=lastDate){dataValuesLethal.push({label:lastDate,value:0});}}else{dataValuesLethal.push({label:lastDate,value:0});}
if(dataValuesSevere.length>0){if(dataValuesSevere[dataValuesSevere.length-1]["label"]!=lastDate){dataValuesSevere.push({label:lastDate,value:0});}}else{dataValuesSevere.push({label:lastDate,value:0});}
if(dataValuesLight.length>0){if(dataValuesLight[dataValuesLight.length-1]["label"]!=lastDate){dataValuesLight.push({label:lastDate,value:0});}}else{dataValuesLight.push({label:lastDate,value:0});}}
$('body').on("jspanelloaded",function(event,id){event.preventDefault();$('body').on("jspanelstatechange",function(event,id){if(id==="statPanel"){if(jsPanelInst!=null){startJSPanelWithChart(jsPanelInst,$("#statPanel").width(),$("#statPanel").height(),$("#statPanel").width()-30,$("#statPanel").height()-80);}}});});$("body").on("jspanelclosed",function closeHandler(event,id){if(id==="statPanel"){jsPanelInst=null;}});